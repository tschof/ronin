...
void from_sigma(dart_order_obj * doj, void *a);


int build_sigma_offsets(struct async_parse_args *apa)
{
...
int on_sigma_enter_order(con_interface * ci, dart_order_obj * doj,
...
int on_sigma_cancel_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
...
void on_sigma_replace_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
    ofp* fix_obj = 0;
...
static void sigma_on_connect(struct connection_interface *ci, int is_connected)
{
    on_connect(ci, is_connected);
}
static ff_desc* create_sigma_ff_out_array(int* len, char* dir, int d_len)
{
...
static ff_desc* create_sigma_ff_in_array(int* len, char* dir, int d_len)
{
        ...
        gatekeeper * gk)
{
    ...
    reset_con_interface(con, 8192,
            ...
            send_fix_logon, build_sigma_offsets,
            on_sigma_enter_order, on_sigma_cancel_order,
            on_sigma_replace_order, gk, (char *) ex->dest_id,
            ex->dest_id_len);
    con->client_mess_parse = parse_fix_message;
    con->ci_parse_v = 0;
    con->on_connect = sigma_on_connect;
    con->ci_obj_callback= from_sigma;
    ...
    int out_len = 0;
    int in_len = 0;
    ff_desc* out_types = create_sigma_ff_out_array(&out_len,directory, ret_len);
    ff_desc* in_types = create_sigma_ff_in_array(&in_len,directory, ret_len);
