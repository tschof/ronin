...
typedef struct dash_parser {
	sbureau_match* algos;
}dash;

void from_dash(dart_order_obj * doj, void *a);

int build_dash_offsets(struct async_parse_args *apa)
{
	//con_interface* con = (con_interface*)apa->ds_book;
	return 0;
}
static void trans_dash_cap(trans_t* t, ofp* fix_obj, char* cap,
		char* mm_home, int mmlen)
{
	switch(cap[0]) {
		case 'B':
			set_fix_val(t, fix_obj, 204, "2", 1);
			break;
		case 'X':
...
static void set_ex_dest(trans_t* t, ofp* fix_obj, char* instr, int instrlen, dart_order_obj* doj)
{

	char* algo = getpval(doj, ROM_ALGO_TYPE);
	int algo_len = getplen(doj, ROM_ALGO_TYPE);
	if(algo_len == 1) {
		switch(algo[0]) {
			case '2':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "VWAP", 4);
				break;
			case '1':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "TWAP", 4);
				break;
			case '9':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "IMPROVE", 7);
				break;
			case '4':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "SMOKE", 5);
				break;
			case '3':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "STRIKE", 6);
				break;
			case '5':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "RETAIL", 6);
				break;
			case '6':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "CLOSE", 5);
				break;
			case '8':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "SENSORDARK", 10);
				break;
			case '7':
				set_fix_val(t, fix_obj, 100, "ALGO", 4);
				set_fix_val(t, fix_obj, 9000, "SENSOR", 6);
				break;
		};
	} else if(algo_len == 2) {
		if(algo[0] == '1' && algo[1] == '0') {
			set_fix_val(t, fix_obj, 100, "ALGO", 4);
			set_fix_val(t, fix_obj, 9000, "BLITZ", 5);
		}
	} else {
		if(instrlen > 0) {
			set_fix_val(t, fix_obj, 100, instr, instrlen);
		}
	}
}
...
void dash_reset_shares_on_replace(dart_order_obj* doj)
{
	...
	dash_calc_leg_ratio(ldoj, doj);
}
static void dash_reset_fields_for_parent(dart_order_obj* orig,
		...
		if(found && lock_n_check(parent)) {
			...
			if(status[0] == '5') {
				...
				if(rep != 0) {
					...
					dash_reset_shares_on_replace(parent);
				}
			}
			if(status_len == 1 && (status[0] == '1' || status[0] == '2')) {
				dash_handle_parent_fill(orig, parent, ci, next_step, status, status_len);
			} else {
				next_step(ci, parent);
				unlock(parent);
			}
...
int on_dash_enter_order(con_interface * ci, dart_order_obj * doj,
		struct async_parse_args *ap)
{
	...
	set_ex_dest(t, fix_obj, getpval(doj,ROM_COPY_INSTR),
			getplen(doj, ROM_COPY_INSTR), doj);
	...
	if(getplen(doj, ROM_CAP) > 0) {
		...
		trans_dash_cap(t, fix_obj, getpval(doj, ROM_CAP), mmhome, mmlen);
	}
	...
	if(getplen(doj, ROM_SHORT_LEND) > 0&& sectype[0] == 'E') {
		...
		set_fix_val(t, fix_obj, 5700, getpval(doj, ROM_SHORT_LEND), getplen(doj, ROM_SHORT_LEND)) ;
	}
	if(doj->childs != 0) {
...
int on_dash_cancel_order(con_interface * ci, dart_order_obj * doj)
{
	trans_t* t = (trans_t*)ci->parser;
...
void on_dash_replace_order(con_interface * ci, dart_order_obj * doj)
{
	trans_t* t = (trans_t*)ci->parser;
	ofp* fix_obj = 0;
	if(doj->childs == 0) {
...
static ff_desc* create_dash_ff_out_array(int* len, char* dir, int d_len)
{
...
static ff_desc* create_dash_ff_in_array(int* len, char* dir, int d_len)
{
...
void create_dash_parser(ex_config* ex, con_interface* con)
{
	dash* gp = calloc(1, sizeof(struct dash_parser));
	int ret_len = 0;
		...
		gatekeeper * gk)
{
	...
	reset_con_interface(con, 8192,
			...
			send_fix_logon, build_dash_offsets,
			on_dash_enter_order, on_dash_cancel_order,
			on_dash_replace_order, gk, (char *) ex->dest_id,
			ex->dest_id_len);
	con->client_mess_parse = parse_fix_message;
	con->ci_parse_v = 0;
	con->on_connect = on_connect;
	con->ci_obj_callback= from_dash;
	...
	int out_len = 0;
	int in_len = 0;
	ff_desc* out_types = create_dash_ff_out_array(&out_len,directory, ret_len);
	ff_desc* in_types = create_dash_ff_in_array(&in_len,directory, ret_len);
	trans_t* t = create_translator(out_types,out_len,
			in_types, in_len);
	con->parser = t;
	con->tg = token_creation_func(con->dest_id, con->dest_len);
	build_children(con);
	create_dash_parser(ex, con);
	...
	if (found && lock_n_check(orig)) {
		...
		switch(type[0]) {
			case '0':
				...
				if(is_spread(orig)) {
					dash_reset_fields_for_parent(orig, doj, ci, fix_return);
				}
				fix_return(ci, orig);
				break;
			case '1':
			case '2':
				{
					...
					if(is_spread(orig)) {
						dash_reset_fields_for_parent(orig, doj, ci, ci->on_fill);
					}
					ci->on_fill(ci, orig);
				}
				break;
			...
			case '4':
				...
				if(is_spread(orig)) {
					dash_reset_fields_for_parent(orig, doj, ci, ci->on_out);
				}
				ci->on_out(ci, orig);
				if(!is_spread(orig)) {
					destroy_orig = 1;
			...
			case '5':
				...
				if(is_spread(orig)) {
					dash_reset_fields_for_parent(orig, doj, ci, ci->on_replaced);
				}
			...
			case '8':
				...
				destroy_gk_db(ci->gk, db);
				if(is_spread(orig)) {
					dash_reset_fields_for_parent(orig, doj, ci, ci->on_reject);
				}
