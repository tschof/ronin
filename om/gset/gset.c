...
static int parse_algo_instructions(dart_order_obj* doj, ofp* fix_obj, trans_t* t,
        gsp* gp)
{
    int is_black = 0;
            ...
            case val:
                if(isspace(*p) != 0 || p == end) {
                    ...
                    if(flen > 0 && val_len > 0) {
                        ...
                        if(val_len == 10 && 
                                strncmp("Blacklight", pval, val_len) == 0){
                            is_black = 1;
                        }
                    }
                }
        ...
        struct async_parse_args *ap)
{
    ...
    no_copy_insert(ci->cl_ord_id_map, getpval(doj, ROM_CLORDID), getplen(doj, ROM_CLORDID),
                   doj);
    pthread_spin_unlock(&ci->mutex);
    set_fix_val(t, fix_obj, 11, getpval(doj, ROM_CLORDID), getplen(doj, ROM_CLORDID));
    ...
    int dlen = getplen(doj, ROM_DESTID);
    if(gp->clen > 0 && dlen == gp->clen &&
            strncmp(dest, gp->is_canada, dlen) == 0) {
        set_fix_val(t, fix_obj, 145, "6", 1);
        set_fix_val(t, fix_obj, 11007, "2", 1);
        set_fix_val(t, fix_obj, 15, "CAD", 3);
        set_fix_val(t, fix_obj, 22, "5", 1);
        if(getplen(doj, ROM_SEC_DEF) > 0) {
            set_fix_val(t, fix_obj, 48,
                    getpval(doj, ROM_SEC_DEF), getplen(doj,
                        ROM_SEC_DEF));
        } else {
            set_fix_val(t, fix_obj, 48,
                    getpval(doj, ROM_SYM), getplen(doj, ROM_SYM));
        }
    } else {
        set_fix_val(t, fix_obj, 145, "1", 1);
        set_fix_val(t, fix_obj, 11007, "1", 1);
    } 
    if(getplen(doj, ROM_COPY_INSTR) > 0) {
        no_eight_thirty_one = parse_algo_instructions(doj, fix_obj, t, gp);
    } 
    if(getplen(doj, ROM_ALGO_TYPE) <= 0) {
        set_fix_val(t, fix_obj, 100, ";", 1);
        if(no_eight_thirty_one == 0) {
            set_fix_val(t, fix_obj, 8031, "SOR", 3);
        }
    } else {
        char* algo = getpval(doj, ROM_ALGO_TYPE);
        switch(algo[0]) {
            case '2':
                set_fix_val(t, fix_obj, 100, "6", 1);
                set_fix_val(t, fix_obj, 8031, "VWAP", 4);
                break;
            case '1':
                set_fix_val(t, fix_obj, 100, "6", 1);
                set_fix_val(t, fix_obj, 8031, "TWAP", 4);
                break;
            case '9':
                set_fix_val(t, fix_obj, 100, ";", 1);
                set_fix_val(t, fix_obj, 8031, "SOR", 3);
            ...
            case '4':
                set_fix_val(t, fix_obj, 100, "6", 1);
                set_fix_val(t, fix_obj, 8031, "SONAR", 5);
                break;
            case '7':
                set_fix_val(t, fix_obj, 100, "47", 2);
                set_fix_val(t, fix_obj, 8031, "SIGMAX", 6);
                break;
        };
    }
...
int on_gset_cancel_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
        ...
        dart_order_obj* obj)
{
    ...
    if(getplen(obj, FIX_EXE_TYPE) > 0) {
        ...
        if(t[0] == '1') {
            t_len = sprintf(title, "Bust from GSCO");
        } else {
            t_len = sprintf(title, "Correction from GSCO");
        }
    } else {
        t_len = sprintf(title, "Unmatched Fill from GSCO");
    }
