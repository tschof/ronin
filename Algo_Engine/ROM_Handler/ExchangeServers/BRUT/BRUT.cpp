// BRUT.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(CBRUTApp, CWinApp)
	//{{AFX_MSG_MAP(CBRUTApp)
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CBRUTApp object

CBRUTApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CBRUTApp initialization

BOOL CBRUTApp::InitInstance()
{
	if (!AfxSocketInit())
	{
	...
	*p1 = '\0';
	...
	//m_Name = "BRUT";
	//m_DatFile = "\\Ordermanager\\Brut.dat";
	...
	pDoc = (CBRUTDoc *)pDocTemplate->CreateNewDocument();
	...
	{
  		m_pClientView = (CBRUTView *)pDoc->GetNextView(pos);
	...
	if ((pos = pDoc->GetFirstViewPosition()))
	{
  		m_pExchangeView = (CBRUTView *)pDoc->GetNextView(pos);
	...
	m_pHandler	= new CBRUTHandler();	// create order handler

	try {
		m_pExchange	= new CBrutFixSession();	// create fix session
	}
	catch(CFixSessionException&) {
		ProcessErrorEx(MODE_ERR_NOTIFY,"BRUT App", "Initialization of FIX session failed.");
		return FALSE;
	}
	
	m_pStatus	= new CBRUTStatus();	// create status handler
...
BOOL CBRUTApp::ProcessClientLogon(CBaseConnection &pC, CCsv& CSV)
{
	CLogin Login;
...
BOOL CBRUTApp::ProcessClientLogout(CBaseConnection &pC, CCsv& CSV)
{
	CLogout Logout;
...
BOOL CBRUTApp::ProcessClientOrder(CBaseConnection& pC, CCsv& CSV)
{
	COrder order(CSV);
...
BOOL CBRUTApp::ProcessClientCancel(CBaseConnection& pC, CCsv& CSV)
{
	CCancel cancel(CSV);
...
BOOL CBRUTApp::ProcessClientReplace(CBaseConnection& pC, CCsv& CSV)
{
...
BOOL CBRUTApp::ProcessClientStatus(CBaseConnection& pC, CCsv& CSV)
{
...
void CBRUTApp::OnHostConnect(CBaseConnection &pC, int nErrorCode)
{
	if (!(nErrorCode))
	{
...
void CBRUTApp::AddHost(CBaseConnection *pHost)
{
	if (pHost)
		m_pExchangeView->m_List.Add(pHost);
}

void CBRUTApp::DeleteHost(CHostConnection *pHost)
{
	if (pHost)
	{
...
void CBRUTApp::UpdateHost()
{
	if (m_pExchangeView)
		m_pExchangeView->m_List.Update();
}
	
void CBRUTApp::ProcessLogEntry(const CString& Name, const CString& Message, char Append)
{
	CMessageApp::ProcessLogEntry(Name, Message, Append);
	//if (m_pStatusView) m_pStatusView->Add(Name + '\t' + Message);
}

void CBRUTApp::ProcessErrorEx(DWORD Mode, const CString& Name, const CString& Error)
{
...
void CBRUTApp::ProcessDebug(const CString& Name, const CString& Debug, char Append)
{
...
void CBRUTApp::SetStatusTimer(UINT Interval)
{
	m_pExchangeView->CreateTimer(Interval);
}

int CBRUTApp::ExitInstance() 
{
	m_pErrorView = NULL;
	m_pStatusView = NULL;
	m_pTrafficView = NULL;
...
void CBRUTApp::OnClearOutgoing()
{
	((CFixSession&)Exchange()).SetNextOutgoingSeqNum(1);
}

void CBRUTApp::OnClearIncoming()
{
	((CFixSession&)Exchange()).SetNextIncomingSeqNum(1);
}

void CBRUTApp::OnLogout() 
{
	Exchange().Logout();
}

void CBRUTApp::UpdateClients()
{
	if (m_pClientView)
