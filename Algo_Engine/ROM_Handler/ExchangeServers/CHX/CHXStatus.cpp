// CHXStatus.cpp: implementation of the CCHXStatus class.
//
//////////////////////////////////////////////////////////////////////
	...
	CStatusHandler()
{
	...
	catch (CDBException *e)
	{
		...
		pApp->ProcessErrorEx(MODE_ERR_NOTIFY,"CHX Status", Error);
		e->Delete();
	}
}

BOOL CCHXStatus::SetStatus(COrder&	Order)
{
	CMessageApp *pApp = (CMessageApp *)AfxGetApp();
...
BOOL CCHXStatus::SetStatus(CCancel& Cancel)
{
	CMessageApp *pApp = (CMessageApp *)AfxGetApp();
...
BOOL CCHXStatus::SetStatus(CCancelReject& Reject)
{
	CMessageApp *pApp = (CMessageApp *)AfxGetApp();
	CString		Trader;
	CString		Owner;
...
BOOL CCHXStatus::SetStatus(CReject& Reject)
{
	BOOL		Result = FALSE;
...
BOOL CCHXStatus::SetStatus(CExecution& Exec)
{
	...
	if (TransType == "0")		// new
	{
		...
		char traderstr[10];
		strcpy(traderstr, Owner);
		if (!(pApp->SendToClient(traderstr, Message.Message())))
		{
...
BOOL CCHXStatus::ProcessTrade(CExecution& Exec)
{
	long		TradeId = -1;
	CString		Error;
	CMessageApp *pApp = (CMessageApp *)AfxGetApp();
	...
	{	
		...
		{
			pApp->ProcessErrorEx(MODE_ERR_BASIC,"CHX Status", "Duplicate Execution Received (" + Exec.Confirm() + "): " + Exec.Debug());
			return(FALSE);
		}
	}
	...
	catch (CDBException *e)
	{
		...
		pApp->ProcessErrorEx(MODE_ERR_DEVELOPER,"CHX Status", Error);
		e->Delete();
