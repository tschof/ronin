...
void CCBOEHandler::InitializeMemoryMap()
{
	...
	try{
		...
		m_pTags = new CCBOEMemTagSet(lDatabaseSettings, forceReload, "rom.dbo.XSRV_CBOE_selDestOrders");
	...
	catch(MMIErrorCodes errorCode)
	{
		...
		pApp->ProcessErrorEx(MODE_ERR_DEVELOPER | MODE_ERR_BUSINESS |MODE_ERR_NOTIFY, "CBOE Exchange", error);
...
void CCBOEHandler::InitializeOrderValidator()
{
	std::string datFile;
	CMessageApp *pApp;
