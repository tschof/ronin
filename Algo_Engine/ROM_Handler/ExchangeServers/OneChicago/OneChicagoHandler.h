// OneChicagoHandler.h: interface for the COneChicagoHandler class.
//
//////////////////////////////////////////////////////////////////////

#if !defined(AFX_OneChicagoHANDLER_H__0BAB0042_9751_4E87_9B36_226FC8D4FCC7__INCLUDED_)
#define AFX_OneChicagoHANDLER_H__0BAB0042_9751_4E87_9B36_226FC8D4FCC7__INCLUDED_
...
class COneChicagoHandler : public CExchangeHandler  
{
public:
	COneChicagoHandler();

	virtual ~COneChicagoHandler()
	{
		delete m_pTags;
...
protected:
	...
	{
		Order.SetSecurityType(COrder::Future);
	...
	{	
		...
		//SPECIAL CASE: Have to cache for replace at this point since "replaced" report doesnt have it from the OneChicago
...
//			Execution.SetExecType(COrder::Replaced);
			...
			//m_pTags->SetReplaceStopPrice(Execution.StopPrice());//OneChicago doesnt send stop price in replaced message so cache it when u send the initial replace out
		...
		else if(Execution.ExecType() == COrder::Canceled ) //Replace processing
		{
			if(0 == Execution.Shares())
			{
				m_pApp->ProcessErrorEx(MODE_ERR_DEVELOPER,m_pApp->Name() +":OneChicagoHandler:OnProcessExecution:", "Received a cancelled message with zero quantity cancelled");
...
//				return FALSE; //ignore cancel since it represents a replaced message which was already taken care of with Open message
		...
		{
			if( Execution.Cumulative() != Execution.Shares() )//this is to cater to the isse where OneChicago sends an incomplete fill i.e. Cum < Original Shares  yet reports it as fill, so we convert it into a partial
	...
	virtual	void OnReplaced(CExecution& Execution)
	{
		...
		m_pTags->SetLastCumQty(0); //since OneChicago does not carry any prevcious CUM_QTY history forward after a replace
	}
...
#endif // !defined(AFX_OneChicagoHANDLER_H__0BAB0042_9751_4E87_9B36_226FC8D4FCC7__INCLUDED_)
