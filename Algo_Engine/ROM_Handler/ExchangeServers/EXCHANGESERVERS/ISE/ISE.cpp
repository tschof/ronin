// ISE.cpp : Defines the class behaviors for the application.
//
...
BEGIN_MESSAGE_MAP(CISEApp, CWinApp)
	//{{AFX_MSG_MAP(CISEApp)
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CISEApp object

CISEApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CISEApp initialization

BOOL CISEApp::InitInstance()
{
	if (!AfxSocketInit())
	{
	...
	pDoc = (CISEDoc *)pDocTemplate->CreateNewDocument();
	...
	{
  		m_pClientView = (CISEView *)pDoc->GetNextView(pos);
	...
	if ((pos = pDoc->GetFirstViewPosition()))
	{
  		m_pExchangeView = (CISEView *)pDoc->GetNextView(pos);
	...
	m_pHandler	= new CISEHandler();								// create order handler
	try {
		m_pExchange	= new CISEFixSession();								// create fix session
	}
	catch(CFixSessionException&) {
		ProcessErrorEx(MODE_ERR_NOTIFY,"ISE App", "Initialization of FIX session failed.");
		return FALSE;
	}
