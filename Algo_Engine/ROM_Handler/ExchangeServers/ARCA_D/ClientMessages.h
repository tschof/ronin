...
namespace {
    ...
    static char m_companyGroupID[ArcaDirect::COMP_GROUP_ID_LEN];

    static std::stringstream message(std::stringstream::binary
                                     | std::stringstream::in 
                                     | std::stringstream::out);
    //static const int OUTPUT_MESSAGE_LEN = 1000;
...
namespace ClientMessages {
    ...
    inline void CreateReplaceMessage(CReplace& order) {
        ...
        const char* szTradeFor = order.lpszLocalAccount();//lpszClearingId();
        memset(m_accountBuff, '\0', ArcaDirect::ACCOUNT_LEN);
        if(szTradeFor && strlen(szTradeFor) < ArcaDirect::ACCOUNT_LEN) {
            strcpy(m_accountBuff, szTradeFor);
        } else if(szTradeFor){
            strncpy(m_accountBuff, szTradeFor, ArcaDirect::ACCOUNT_LEN);       
        }
        
        message.write(m_accountBuff, ArcaDirect::ACCOUNT_LEN);
        output <<" Account: "<<m_accountBuff<<"  -- DONE--";
    ...
    inline void CreateOrderMessage(COrder& order) {
        ...
        //Set the company ID. This value is loaded from the dat file.
        ...
        /*Setting the clearing ID in the Account field.*/
        const char* szTradeFor = order.lpszClearingAccount();//.lpszTradeFor();//order.lpszClearingId();
        memset(m_accountBuff, '\0', ArcaDirect::ACCOUNT_LEN);
        if(szTradeFor && strlen(szTradeFor) < ArcaDirect::ACCOUNT_LEN) {
            strcpy(m_accountBuff, szTradeFor);
        } else if(szTradeFor){
