...
*
*******************************************************************************/
#ifndef ARCADIRECTMESSAGE_H__
#define ARCADIRECTMESSAGE_H__
...
namespace ArcaDirect {
    ...
    static const short LOGON_REJECT_LEN = 56;
    ...
    static const short ORDER_FILL_LEN = 32;
    ...
    inline void SetPriceFields(std::stringstream& message,
                               double price) {
        double tPrice = price * 1000;
        if(price >=0) {
            tPrice += .05;
        } else {
            tPrice += -.05;
        }
        unsigned int newPrice = static_cast<unsigned int>(tPrice);
        newPrice /= 10;
        unsigned int ndPrice = htonl(newPrice);
        message.write((char*)&ndPrice, sizeof(ndPrice));
        message.put('2');
    }
    /**
    ...
    inline int ProcessMessage(std::stringstream& stream,
        ...
        } else if(exe.Status() == COrder::ReplacePending) {
            //exe.SetOrderTag(idStr);
            exe.SetCancelTag(idStr);
        } else {
            exe.SetOrderTag(idStr);
    ...
    inline int ProcessOrderFilledMessage(std::stringstream& stream,//const char* message,
        ...
        char side;
        stream.get(side);
        exe.SetSide(ArcaDirectMessage::ToStaffordSide(side));
