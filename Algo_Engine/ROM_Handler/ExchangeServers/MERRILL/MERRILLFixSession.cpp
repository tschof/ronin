 ...
 *******************************************************************************/
/*
 * MERRILLFixSession.cpp: implementation of the CMERRILLFixSession class.
 *
 *
 * History
 *
 ...
 * 10/24/05	   joel        Fix bug, set ExDestination field if route not set to Direct+
 *
 *******************************************************************************/
...
CMERRILLFixSession::CMERRILLFixSession()
{
...
void CMERRILLFixSession::InitializeFixCustomTags()
{
...
//	GetPrivateProfileString("Fix Information", "OnBehalfOfCompIDMERRILL", "", (char*) l_Buf.c_str(), 8,l_FileName.c_str()); 
...
/*
void CMERRILLFixSession::ParseOnBehalfOfCompIDMap()
{
...
void CMERRILLFixSession::ParseOnBehalfOfCompID( const std::string & str, std::vector<std::string> & fields, char token, char end  )
{
...
std::string CMERRILLFixSession::trim( const std::string & str )
{
...
void CMERRILLFixSession::Process(const CFixLogout& a_Logout)
{
...
long CMERRILLFixSession::ProcessEntry(COrder& a_Order)
{
    if(m_AwaitingResend)
    {
...
BOOL CMERRILLFixSession::OrderEntryOverride(const COrder& a_Order, CFixOrder& a_FixOrder)
{
	...
	{
		CString Msg("AFS:OEO:MERRILL supports Buy/Sell/Short/ShortExempt sides only.");
		throw Msg;
	}
...
BOOL CMERRILLFixSession::ProcessCancel(CCancel& a_Cancel)
{
...
BOOL CMERRILLFixSession::CancelOverride(const CCancel &a_Cxl, CFixCancel& a_FixCxl)
{
...
BOOL CMERRILLFixSession::ProcessReplace(CReplace& a_Replace)
{
	if(m_AwaitingResend)
	{
...
BOOL CMERRILLFixSession::ReplaceOverride(const CReplace &a_Replace, CFixReplace& a_FixReplace)
{
	//a_FixReplace.SecurityType("CS");

	if(a_FixReplace.Side() != FIX_Buy && a_FixReplace.Side() != FIX_Sell && a_FixReplace.Side() != FIX_SellShort)
	{
		CString Msg("AFS:RO:MERRILL supports Buy/Sell/Short sides only.");
		throw Msg;
	}
	...
	if (a_Replace.ClearingAccount().IsEmpty() == 0)
	{
		a_FixReplace.Account(a_Replace.ClearingAccount());
	}
...
void CMERRILLFixSession::Process(const CFixExecution& a_Exec)
{
	...
	if(l_TransType == FIX_TransType_Cancel)
	{
		...
		((CMessageApp*)AfxGetApp())->ProcessErrorEx(MODE_ERR_BUSINESS,"MERRILL:Busted Stock Trade", l_MsgStr);
		return;
	...
	else if(l_TransType == FIX_TransType_Correct)
	{
		...
		l_MsgStr.Format("Exchange order ID: %s\nAffected Execution Ref ID: %s\nSide: %s\nOriginal Shares: %d\nSymbol: %s\nOrder Type: %s\nOriginal Price: %f\nTime-in-Force: %s\nLast Shares: %d\nLast Price: %f\nCorrection Time (GMT): %s",
						...
						a_Exec.LastPx(),
						a_Exec.SendingTime().c_str());
		((CMessageApp*)AfxGetApp())->ProcessErrorEx(MODE_ERR_BUSINESS,"MERRILL:Stock Trade Correction", l_MsgStr);
		return;
	}
...
BOOL CMERRILLFixSession::ExecutionOverride(CExecution &Exec, const CFixExecution& a_Exec)
{
...
void CMERRILLFixSession::ProcessCMTAGiveup(CString& GiveupAccount, CString& CMTAAccount, CFixOrder& a_FixOrder, const COrder& a_Order)
{
	if (GiveupAccount != "")
	{
...
void CMERRILLFixSession::ProcessCMTAGiveup(CString& GiveupAccount, CString& CMTAAccount, CFixReplace& a_FixReplace, const CReplace& a_Replace)
{
	if (GiveupAccount != "")
	{
