...
TposTrade :: TposTrade(TradeDesc* val)
{
	...
	m_LastModReasonID = val->getLastModReasonID();
...
TposPosition :: TposPosition(PositionDesc* val)
{
	...
	m_InstrumentID = gcnew String(val->getPosInstID().c_str());
	m_ValidTraderKey = gcnew String(val->getAccountTraderID().c_str());
	m_DisplayFormat = gcnew String(val->getDisplayFormat().c_str());
	m_Multiplier = val->getMultiplier();
	m_AvgCost = val->getAvgCost();
	m_RealizedPnL = val->getRealizedPNL();
	m_OpenMark = val->getClosingMark();
		
	m_NotionalAmount = val->getNotionalAmount();
}
...
void TposPositionCallback :: OnPosition(TposPosition^ position)
{
	printf("Original OnPosition Callback\n");
}

void TposPriceCallback :: OnPrice(TposPrice^ price)
{
	printf("Original OnPrice Callback\n");
}