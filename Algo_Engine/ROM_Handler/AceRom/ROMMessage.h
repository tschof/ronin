...
private:
    ...
    inline void ParseMessage(const std::string& l_sub) {
		...
		std::string::size_type delimPos = ll_sub.find(DELIMITER, subOff);
		while (subOff < length && count < (ROMMSG_INIT_LEN -1) && delimPos != std::string::npos) {
            std::string sub = ll_sub.substr(subOff, (delimPos - subOff));
            SetAt(count, sub);
            ++count;
            subOff = delimPos + 1;
		...
		if(subOff < length) {
			std::string sub = ll_sub.substr(subOff);
			SetAt(count, sub);
		}
    }
	...
	inline void ParseMessage(const char* pBuf, int len) 
		{
			...
			if(pCur != NULL) 
			{
				length = pCur - pBuf;
				char* pChild = pCur + 3;
				while( (pCur = strstr(pChild, "!#!")) != NULL) 
				{
					CRCsv* pCsv = new CRomMessage(pChild, pCur - pChild);
					m_children.push_back(pCsv);
					pChild = pCur + 3;
				}
			}
			...
			while ((pTmp = strchr(pCur, DELIMITER)) != NULL &&
					count < ROMMSG_INIT_LEN -1)
			{
				std::string sub(pCur, pTmp - pCur);
				SetAt(count++, sub);
				pCur = pTmp + 1;
			}
