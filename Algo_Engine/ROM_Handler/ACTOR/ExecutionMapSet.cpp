...
BOOL CExecutionMapSet::Lookup(const CString& Confirm)
{
	BOOL Found = FALSE;

	m_Sql = "select [ExchangeTag],[StaffordTag],[OrderTag],[ExecutionTag],[Date] from " + m_TableName + " where [ExecutionTag] = '" + Confirm + "' AND [Date]='" + GetDate() + "'";
