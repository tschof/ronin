...
BOOL CTerraNovaApp::InitInstance()
{
	...
	if(short Cards = HIWORD(XAPIGetCards()))
	{
	...
	// create exchange window
		pDoc = (CTerraNovaDoc *)pDocTemplate2->CreateNewDocument();
		pNewFrame = (CConnectionFrame *)(pDocTemplate2->CreateNewFrame(pDoc, NULL));
		pNewFrame->SetTitle("Exchange Connection");
		pDocTemplate->InitialUpdateFrame(pNewFrame, pDoc);
		pNewFrame->Restore();
		if ((pos = pDoc->GetFirstViewPosition()))
			{
  			m_pExchangeView = (CTerraNovaView2 *)pDoc->GetNextView(pos);
...
void	CTerraNovaApp::ProcessLogEntry(const CString& Name, const CString& Entry)
{
	CString tracestream;
	if (m_Log.m_pStream)
		{
