...
namespace LBMEx
{
	...
	internal class LBM : MamaDictionaryCallback, MamaSubscriptionCallback, MamaMsgFieldIterator, MamdaOrderBookHandler, IDisposable
	{
		...
		{
			// Check for troublesome message types

			//Debug.Print(msg.getType().ToString());
			switch (msg.getType())
			{
			...
			// Check for troublesome message statuses
			Debug.Print(msg.getStatus().ToString());
			...
			else
			{
				...
				if (issueSymbol != "")
				{
					msg.iterateFields(this, _mamaDictionary, new string[] { issueSymbol, subscription.subscSymbol, subscription.subscSource });
				}
				...
				if (!_threadMDChangesStarted)
				{
					...
					_threadMDChanges.Priority = System.Threading.ThreadPriority.BelowNormal;
		...
		public void onField(MamaMsg msg, MamaMsgField field, object closure)
		{
			...
			lock (_syncObj)
			{
				if (!_mamaOnFieldChanges.ContainsKey(symbol))
				{
					string participentSymbol = "";

					_mamaOnFieldChanges.Add(symbol, new MDServerToClient());

					DecodeIssueSymbol(ref symbol, ref participentSymbol);

					_mamaOnFieldChanges[symbol].Update(WombatFieldIDs.IssueSymbol, symbol);
					_mamaOnFieldChanges[symbol].Update(WombatFieldIDs.uSymbol, symbol);
					_mamaOnFieldChanges[symbol].Update(WombatFieldIDs.uParticipentSymbol, participentSymbol);

					_mamaOnFieldChanges[symbol].Update(WombatFieldIDs.uSubscriptionSymbol, strings[1]);
					_mamaOnFieldChanges[symbol].Update(WombatFieldIDs.uSubscriptionSoruce, strings[2]);
				}
				...
				if (MDOutputFieldIDs.FNames.ContainsKey(fid))
				{
					switch (field.getType())
					{
						case mamaFieldType.MAMA_FIELD_TYPE_STRING:
							_mamaOnFieldChanges[symbol].Update(fid, field.getString());
							break;
						case mamaFieldType.MAMA_FIELD_TYPE_F64:
							_mamaOnFieldChanges[symbol].Update(fid, Convert.ToDouble(field.getF64()));
							break;
						case mamaFieldType.MAMA_FIELD_TYPE_I64:
							_mamaOnFieldChanges[symbol].Update(fid, Convert.ToInt64(field.getI64()));
							break;
						case mamaFieldType.MAMA_FIELD_TYPE_I32:
							_mamaOnFieldChanges[symbol].Update(fid, Convert.ToInt64(field.getI32()));
							break;
						//case mamaFieldType.MAMA_FIELD_TYPE_PRICE:
						//    _mamaOnFieldChanges[symbol].Update(fid, field.getPrice().getValue());
						//    break;
						//case mamaFieldType.MAMA_FIELD_TYPE_I8:
						//case mamaFieldType.MAMA_FIELD_TYPE_CHAR:
						//    _mamaOnFieldChanges[symbol].Update(fid, field.getChar().ToString());
						//    break;
						//case mamaFieldType.MAMA_FIELD_TYPE_BOOL:
						//    _mamaOnFieldChanges[symbol].Update(fid, Convert.ToInt32(field.getBool()));
						//    break;
						//case mamaFieldType.MAMA_FIELD_TYPE_U8:
						//    _mamaOnFieldChanges[symbol].Update(fid, field.getU8());
						//    break;
						//case mamaFieldType.MAMA_FIELD_TYPE_I16:
						//    _mamaOnFieldChanges[symbol].Update(fid, field.getI16());
						//    break;
						//case mamaFieldType.MAMA_FIELD_TYPE_U16:
						//    _mamaOnFieldChanges[symbol].Update(fid, field.getU16());
						//    break;
						//case mamaFieldType.MAMA_FIELD_TYPE_I32:
						//    _mamaOnFieldChanges[symbol].Update(fid, field.getI32());
						//    break;
						//case mamaFieldType.MAMA_FIELD_TYPE_U32:
						//    _mamaOnFieldChanges[symbol].Update(fid, field.getU32());
						//    break;
						//case mamaFieldType.MAMA_FIELD_TYPE_U64:
						//    _mamaOnFieldChanges[symbol].Update(fid, field.getU64());
						//    break;
						default:
			...
			//lock (_syncObj)
			//{
			//    _mamaOnFieldChanges[symbol].Update(c);
			//}
		}
		...
		private void ProcessMDChangeQueueAndNotify()
		{
			...
			{
				Dictionary<object, MDServerToClient> locCopy = new Dictionary<object, MDServerToClient>();
				lock (_syncObj)
				{
					if (_mamaOnFieldChanges.Count > 0)
					{
						locCopy = new Dictionary<object, MDServerToClient>(_mamaOnFieldChanges);
				...
				if (locCopy.Count > 0)
				{
					OnLBMChanged(new LBMDataChangedEventArgs(LBMDataChangedEventTypes.OnSubscriptionMsg, ConvertToList(locCopy)));
				}
