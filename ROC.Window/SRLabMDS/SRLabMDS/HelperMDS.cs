	...
	public class HelperMDS : HelperStatusBase
	{
			...
			if (Configuration.MDS.Default.EnableTCPLog)
			{
				if (client != null)
				{
					lock (SentMsgs)
					{
						...
						SentMsgs.Add(String.Concat(new object[] { _dtHP.Now.ToString("hh:mm:ss.ffffff"), " ", LastSentMsg }));
					}
				}
			}
		}
		...
		private Dictionary<string, byte[]> ConvertToRecapBytes(Dictionary<string, List<IntPtr>> locRecapQueue)
		{
			...
			lock (GLOBAL.HSRLab.LatestData)
			{
				...
				{
					if (GLOBAL.HSRLab.LatestData.ContainsKey(key))
					{
						locSymbolToRecapMap.Add(key, ConvertToOutput(GLOBAL.HSRLab.LatestData[key]));
						PerformanceLog("MDS|ProcessRecapQueue", string.Format("{0,-12}|[{1,-3}]Deltas", "[1]" + key, GLOBAL.HSRLab.LatestData[key].Count));
					}
					else
					{
		...
		private void SetReceivedMsg(Socket client, string receivedMsg)
		{
			if (Configuration.MDS.Default.EnableTCPLog)
			{
				lock (ReceivedMsgs)
				{
					...
					ReceivedMsgs.Add(String.Concat(new object[] { _dtHP.Now.ToString("hh:mm:ss.ffffff"), " ", LastReceivedMsg }));
				}
			}
		}
			...
			lock (e.Client)
			{
				if (e.Client.Connected && ClientToSymbolMap.ContainsKey(e.Client.Handle))
				{
					...
					if (e.ByteMessage != null && e.ByteMessage.Length > 0)
					{
						...
						if (!Configuration.MDS.Default.EnableClientReload)
						{
							clientMsg.Reload = false;
						}
		...
		private void SubscribeToSymbol(IntPtr clientKey, MDClientToServer symbolMap)
		{
			...
			if (!IsRecap(clientKey, symbolMap))
			{
				...
				GLOBAL.HSRLab.Subscribe(symbolMap);
			}
		}
		...
		private bool IsRecap(IntPtr clientKey, MDClientToServer symbolMap)
		{
			...
			lock (GLOBAL.HSRLab.LatestData)
			{
				if (GLOBAL.HSRLab.LatestData.ContainsKey(symbolMap.MDKey))
				{
					recap = true;

					latestDataKeys = new string[GLOBAL.HSRLab.LatestData.Count];
					GLOBAL.HSRLab.LatestData.Keys.CopyTo(latestDataKeys, 0);
					latestDataKeyList = new List<string>(GLOBAL.HSRLab.LatestData.Keys);
				}
			}
		...
		public void AddToMDSLog(string msg)
		{
			if (Configuration.MDS.Default.EnableMDSLog)
			{
				lock (SyncObj)
				{
					MDSLogs.Add(String.Concat(new object[] { _dtHP.Now.ToString("hh:mm:ss.ffffff"), " ", msg }));
				}
			}
		}
