...
namespace FormEx
{
	public class XWindow : Form
	{
        ...
        #region - XWindowHolderButtonClickArgs -
        ...
        public class XCoolFormHolderButtonClickArgs : EventArgs
        {
            ...
            public XCoolFormHolderButtonClickArgs(int lIndex)
				: base()
            {
                m_lIndex = lIndex;
            }
        ...
        public XWindow()
        {
			...
			this.Padding = new Padding(8, 38, 8, 35);
            ...
            if (PointInRect(e.Location, m_rcSizeGrip))
            {
                ...
                if (m_bMouseDown && bResizing)
                {
                    ...
                    if (e.Button == MouseButtons.Left)
                    {
						...
						DockWindowHelper. SendMessage(Handle, DockWindowHelper.WM.WM_SYSCOMMAND, (IntPtr)(DockWindowHelper.SC.SC_SIZE + 8), IntPtr.Zero);
        ...
        private void FillMenuIconGradient(GraphicsPath XFillPath, Graphics g, List<Color> mix)
        {
            ...
            using (XAntiAlias xaa = new XAntiAlias(g))
            {
                     ...
                     LinearGradientMode.Vertical))
                {
                    ...
                    lgb.InterpolationColors = XWidnowHelper.ColorMix(mix, false);
