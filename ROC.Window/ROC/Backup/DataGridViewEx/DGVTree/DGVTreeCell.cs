...
{
	public class DGVTreeCell : DataGridViewTextBoxCell
	{
		...
		{
			DGVTreeCell c = (DGVTreeCell)base.Clone();
		...
		public int Level
		{
			get
			{
				DGVTreeNode row = this.OwningNode;
				if (row != null)
				{
					return row.Level;
				}
		...
		protected override void Paint(Graphics graphics, Rectangle clipBounds, Rectangle cellBounds, int rowIndex, DataGridViewElementStates cellState, object value, object formattedValue, string errorText, DataGridViewCellStyle cellStyle, DataGridViewAdvancedBorderStyle advancedBorderStyle, DataGridViewPaintParts paintParts)
		{
			...
			DGVTreeNode node = this.OwningNode;
			if (node == null) return;
				...
				using (Pen linePen = new Pen(SystemBrushes.ControlDark, 1.0f))
				{
					...
					if (node.Level == 1)
					{
						...
						if (isFirstSibling && isLastSibling)
						{
							...
							graphics.DrawLine(linePen, glyphRect.X + 4, cellBounds.Top + cellBounds.Height / 2, glyphRect.Right + 4, cellBounds.Top + cellBounds.Height / 2);
						}
					...
					else
					{
						...
						DGVTreeNode previousNode = node.Parent;
					...
					{
						// paint lines of parent to child
						graphics.DrawLine(linePen, glyphRect.Right + 4, cellBounds.Top + cellBounds.Height / 2, glyphRect.Right + 4, cellBounds.Bottom);
					}
				}
			}
		...
		public DGVTreeNode OwningNode
		{
			get { return base.OwningRow as DGVTreeNode; }
		}
	}
}
