...
namespace DataGridViewEx
{
	...
	public class DGVTreeNode : DataGridViewRow//, IComponent
	{
		internal DGVTree _grid;
		internal DGVTreeNode _parent;
		internal DGVTreeNodeCollection _owner;
		...
		DGVTreeCell _treeCell;
		DGVTreeNodeCollection childrenNodes;
		...
		internal DGVTreeNode(DGVTree owner)
			: this()
		{
			this._grid = owner;
		...
		{
			DGVTreeNode r = (DGVTreeNode)base.Clone();
			r.UniqueValue = -1;
		...
		void cells_CollectionChanged(object sender, System.ComponentModel.CollectionChangeEventArgs e)
		{
			...
			{
				DGVTreeCell treeCell = null;

				if (e.Element == null)
				{
					foreach (DataGridViewCell cell in base.Cells)
					{
						if (cell.GetType().IsAssignableFrom(typeof(DGVTreeCell)))
						{
							treeCell = (DGVTreeCell)cell;
							break;
						}
				...
				{
					treeCell = e.Element as DGVTreeCell;
				}
		...
		public DGVTreeNodeCollection Nodes
		{
			get
			{
				if (childrenNodes == null)
				{
					childrenNodes = new DGVTreeNodeCollection(this);
				}
		...
		public int Level
		{
			get
			{
				if (this._level == -1)
				{
					// calculate level
					int walk = 0;
					DGVTreeNode walkRow = this.Parent;
					while (walkRow != null)
					{
						walk++;
		...
		public bool IsLastSibling
		{
			get
			{
				DGVTreeNode parent = this.Parent;
		...
		internal protected virtual bool InsertChildNode(int index, DGVTreeNode node)
		{
			node._parent = this;
		...
		internal protected virtual bool InsertChildNodes(int index, params DGVTreeNode[] nodes)
		{
			foreach (DGVTreeNode node in nodes)
			{
		...
		internal protected virtual bool AddChildNode(DGVTreeNode node)
		{
			node._parent = this;
			node._grid = this._grid;
		...
		internal protected virtual bool AddChildNodes(params DGVTreeNode[] nodes)
		{
		...
		internal protected virtual bool RemoveChildNode(DGVTreeNode node)
		{
		...
		private void UpdateChildNodes(DGVTreeNode node)
		{
			if (node.HasChildren)
			{
				foreach (DGVTreeNode childNode in node.Nodes)
				{
