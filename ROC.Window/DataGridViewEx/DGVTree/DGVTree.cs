...
namespace DataGridViewEx
{
	...
	public class DGVTree : DataGridView
	{
		//private int _indentWidth;
		private DGVTreeNode _root;
		private DGVTreeColumn _expandableColumn;
		...
		public DGVTree()
		{
			...
			this.RowTemplate = new DGVTreeNode() as DataGridViewRow;
			// This sample does not support adding or deleting rows by the user.
			this.AllowUserToAddRows = false;
			this.AllowUserToDeleteRows = false;
			this._root = new DGVTreeNode(this);
		...
		public DGVTreeNode GetNodeForRow(DataGridViewRow row)
		{
			return row as DGVTreeNode;
		}

		[Description("Returns the TreeGridNode for the given DataGridViewRow")]
		public DGVTreeNode GetNodeForRow(int index)
		{
		...
		public new DGVTreeNode CurrentRow
		{
			get
			{
				return base.CurrentRow as DGVTreeNode;
			}
		}
		...
		public DGVTreeNode CurrentNode
		{
			get
			{
		...
		public new int RowCount
		{
			...
			set
			{
				for (int i = 0; i < value; i++)
					this.Nodes.Add(new DGVTreeNode());
		...
		protected override void OnRowsAdded(DataGridViewRowsAddedEventArgs e)
		{
			...
			while (count >= 0)
			{
				row = base.Rows[e.RowIndex + count] as DGVTreeNode;
				if (row != null)
				{
					row.Sited();
				}
		...
		internal protected virtual void UnSiteNode(DGVTreeNode node)
		{
		...
		internal protected virtual bool CollapseNode(DGVTreeNode node)
		{
			if (node.IsExpanded)
			{
		...
		internal protected virtual void SiteNode(DGVTreeNode node)
		{
			//TODO: Raise exception if parent node is not the root or is not sited.
			int rowIndex = -1;
			DGVTreeNode currentRow;
			node._grid = this;
			...
			if (currentRow != null)
			{
				while (currentRow.Level >= node.Level)
				{
					...
					{
						currentRow = base.Rows[currentRow.RowIndex + 1] as DGVTreeNode;
		...
		internal protected virtual void SiteNode(DGVTreeNode node, int index)
		{
		...
		internal protected virtual bool ExpandNode(DGVTreeNode node)
		{
		...
		{
			if (typeof(DGVTreeColumn).IsAssignableFrom(e.Column.GetType()))
			{
				if (_expandableColumn == null)
				{
					// identify the expanding column.			
					_expandableColumn = (DGVTreeColumn)e.Column;
				}
				else
				{
