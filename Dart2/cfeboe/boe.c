...
static void int_audit_log(boem* pc, char* raw, int rlen)
{
    if(rlen > 0) {
        //record_audit_trail(pc->o, raw, rlen,cfe_audit_trail, pc);
    }
}
                      ...
                      boem* con)
{
    ...
    unsigned short param_len = 13;
    unsigned char type = 0x25;
    unsigned char bits = 0x8;
    ...
    databuf_memcpy(buff, &zero, 1);
    databuf_memcpy(buff, &zero, 1);
    unsigned char oak5 = 0x0;
    SET_BIT(0x1, 0x40, oak5);
    unsigned char oak7 = 0x0;
    SET_BIT(0x1, 0x1, oak7);
    databuf_memcpy(buff, &oak5, 1);
    databuf_memcpy(buff, &zero, 1);
    databuf_memcpy(buff, &oak7, 1);
    databuf_memcpy(buff, &zero, 1);
    ord->num_params += 1;
    ...
    databuf_memcpy(buff, &bits, 1);
    SET_BIT(0x1, 0x1, om1);
    /*SET_BIT(0x1, 0x2, om1); PEG DIFF*/
    SET_BIT(0x1, 0x4, om1);
    /*SET_BIT(0x1, 0x8, om1); ExecInst*/
    SET_BIT(0x1, 0x10, om1);
    SET_BIT(0x1, 0x20, om1);
    /*SET_BIT(0x1, 0x40, om1); MaxRemovePct */
    databuf_memcpy(buff, &om1, 1);
    databuf_memcpy(buff, &om2, 1);
    /*SET_BIT(0x1, 0x8, om3); DisplayIndicator */
    /*SET_BIT(0x1, 0x10, om3); MaxFloor */
    ...
    SET_BIT(0x1, 0x1, om5);
    SET_BIT(0x1, 0x2, om5);
    /*SET_BIT(0x1, 0x10, om5);DisplayPrice
    SET_BIT(0x1, 0x20, om5); WorkingPrice*/
    /*SET_BIT(0x1, 0x80, om5); ExpireTime */
    databuf_memcpy(buff, &om5, 1);
    databuf_memcpy(buff, &om6, 1);
    databuf_memcpy(buff, &om7, 1);
    SET_BIT(0x1, 0x4, om8);
    /*SET_BIT(0x1, 0x8, om8); RoutingInst
    SET_BIT(0x1, 0x10, om8); RoutStrategy
    SET_BIT(0x1, 0x20, om8); RoutDeliveryMethod */
    ...
    databuf_memcpy(buff, &om11, 1);
    om12 = 0x0;
    SET_BIT(0x1, 0x10, om12);
    SET_BIT(0x1, 0x40, om12);
    databuf_memcpy(buff, &om12, 1);
    om13 = 0x0;
    databuf_memcpy(buff, &om13, 1);
    ord->num_params += 1;
...
void send_boe_logon(void* c)
{
    ...
    send_debug_message(bm->o, "Sent Logon to CFE \n");
}
...
int on_boe_enter_order(con_int * ci, rcntr* cnt)
{
    ...
    if(cnt->rlen == sizeof(dart_order_obj)) {
        ...
        if (total_len > 0) {
            ...
            int_audit_log(bm, buff->rd_ptr, databuf_unread(buff));
        } else if(total_len <= 0) {
            ret = -1;
        }
...
static void boe_on_connect(void* pass_back, int is_connected,
                           dart_order_obj* log)
{
                             ...
                             short mlen, long bu)
{
    ...
    int_audit_log(pc, (char*)message, mess_len);
    uint8_t munit = 0x0;
    uint32_t seq = 0x0;
        ...
        switch(mtype) {
            ...
            case 0x2C: //execution
                ...
                if (ord_done == 1) {
                    ...
                    set_rom_field(doj, ROM_SPREAD, "5", 1);
                    if(sender && sender->on_fill) {
                        sender->on_fill(sender, obj);
                    } else if(sender) {
                       log_for_sender(ci, sender, obj); 
                    }
                    set_rom_field(doj, ROM_SPREAD, "6", 1);
                }
                if(sender && sender->on_fill) {
                    sender->on_fill(sender, obj);
                } else if(sender) {
                ...
                break;
            case 0x49:
                send_debug_message(ci->o, "Got a TAS restatement: %s, with type: %c \n", clordid, mtype);
                boe_parse_tas_restate(obj, message, ci);
                sender = get_sender(pc,doj,obj);
                if(sender && sender->on_fill) {
                    sender->on_fill(sender, obj);
                }
