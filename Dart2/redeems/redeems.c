...
void from_redeems(void* a, dart_order_obj * doj);



static void trans_redeems_cap(ofp* fix_obj, char* cap)
{
        switch(cap[0]) {
            case 'A':
...
int on_redeems_enter_order(con_int * ci, rcntr * risk)
{
    ...
    set_fix_val(fix_obj, 100, "ABNY", 4);
    set_fix_val(fix_obj, 40, "M", 1);
    ...
    if(getplen(doj, ROM_CAP) > 0) {
        trans_redeems_cap(fix_obj, getpval(doj, ROM_CAP));
    }
...
int on_redeems_cancel_order(con_int * ci, rcntr * risk)
{
    dart_order_obj* doj = (dart_order_obj*)risk->raw;
    fixp* fp = (fixp*)ci->cparser;
    trans_t* t = get_trans(fp);
...
int on_redeems_replace_order(con_int * ci, rcntr * risk)
{
    dart_order_obj* doj = (dart_order_obj*)risk->raw;
    fixp* fp = (fixp*)ci->cparser;
    trans_t* t = get_trans(fp);
...
static void redeems_logout(con_int* ci)
{
    fixp* fp = (fixp*)ci->cparser;
...
static void redeems_on_connect(void *v, int is_connected, dart_order_obj* doj)
{
    fixp* fp = (fixp*)v;
...
static ff_desc* create_redeems_ff_out_array(int* len, char* dir, int d_len)
{
...
static ff_desc* create_redeems_ff_in_array(int* len, char* dir, int d_len)
{
        ...
        gatekeeper * gk)
{
    ...
    build_children(con);
    con->sbm = 0;
    con->on_enter_order = on_redeems_enter_order;
    con->on_replace_order = on_redeems_replace_order;
    con->on_cancel_order = on_redeems_cancel_order;
    ...
    int out_len = 0;
    int in_len = 0;
    ff_desc* out_types = create_redeems_ff_out_array(&out_len,con->ex->directory, con->ex->dir_len);
    ff_desc* in_types = create_redeems_ff_in_array(&in_len,con->ex->directory, con->ex->dir_len);
    ...
    fixp* fp = create_fix_parser(con,
                                 from_redeems,d, con->o, t, con->ex->order_allocation);
    set_fp_header_len(fp, 13);
    set_log_info(fp, li);
    rex_con* rc = create_rex_con(con->p, con->ex, fp, fp,
                                 li,fp_write_callback, 
                                 redeems_on_connect, parse_fix_message, NULL);
    set_rex_con(fp, rc);
    con->cleansing_rinse = redeems_logout;
    rc->parser = fp;
    con->cparser = fp;
                                 ...
                                 con_int* ci)
{
    if(doj->childs != 0) {
        ...
        while(lch != NULL) {
            ...
            reset_rom_field(ldoj, 0, ROM_ZERO_STATUS, 1);
            reset_rom_field(ldoj, ROM_STATUS, "8", 1);
...
{
    send_biz_message(getpval(doj, 26), getplen(doj, 26), "News Flash from the ABN!",
                     24);
}
