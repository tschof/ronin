...
module FilePackager =
    let writeOutFiles(imi:PantherCore.IMConfig,s3Client: IAmazonS3,context: ILambdaContext,
                       jConfig:PantherCore.CatConfig.Root, stuff:byte[], blen:int64, recordCount:int, fileNum:int) =
        ...
        let hash = PantherCore.computeHash(outMemoryStream)
        let fReqest = new Amazon.S3.Model.PutObjectRequest()
        fReqest.BucketName <- jConfig.FinishedBucket
        fReqest.Key <-jConfig.CatSubmitterId.ToString() + "_" + jConfig.CatReporterImid + "_" + PantherCore.createDateNGroup(imi.imid, fileNum) + ".csv.bz2"
        ...
        let meta = new PantherCore.MetaDataC([PantherCore.MetaDataC.Row("META", "true", date, jConfig.CatReporterImid, jConfig.CatSubmitterId.ToString(), "2.2.1", r, None)])
        ...
        mRequest.Key <- jConfig.CatSubmitterId.ToString() + "_" + jConfig.CatReporterImid + "_" + PantherCore.createDateNGroup(imi.imid, fileNum) + ".meta.csv"
        mRequest.InputStream <- memWrite
        s3Client.PutObjectAsync(mRequest).Result

    let processFile(imids:seq<PantherCore.IMConfig>,s3Client: IAmazonS3,context: ILambdaContext,
                    jConfig:PantherCore.CatConfig.Root, rows:System.IO.BinaryReader, fileNum:int) =
        sprintf "In process File %d" (Seq.length imids) |> context.Logger.LogLine
        imids |> Seq.iter(fun elem -> let (memBuff, bLen,records) = CboeBinReader.buildOptionFile(rows, elem)
...
type Function(s3Client: IAmazonS3) =
    ...
    member __.FunctionHandler (event: S3Event) (context: ILambdaContext) =
        let fetchContentType (s3Event: S3EventNotification.S3Entity) = async {
            ...
            let jConfig = PantherCore.CatConfig.Parse(configReader.ReadToEnd())
            ...
            let lRunDate = PantherCore.FileCounter.Parse(runReader.ReadToEnd())
            ...
            let reader = new System.IO.BinaryReader(resp.ResponseStream)   
          
       
            let imids = PantherCore.getIMIDS(jConfig.AccountToImid)
            let fileOff = PantherCore.getCurrentFileNum(lRunDate)
            FilePackager.processFile(imids,s3Client,context,jConfig, reader, fileOff)
            ...
            let newLastRunFile = PantherCore.resetLastRunConfig(fileOff)
