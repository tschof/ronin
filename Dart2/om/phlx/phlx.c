...
void from_phlx(dart_order_obj * doj, void *a);

int build_phlx_offsets(struct async_parse_args *apa)
{
...
static void trans_phlx_cap(trans_t* t, ofp* fix_obj, char* cap,
                           char* mm_home, int mmlen, dart_order_obj* doj)
{
        switch(cap[0]) {
            case 'B':
            ...
            case 'X':
                ...
                break;
            case 'S':
                set_fix_val(t, fix_obj, 204, "4", 1);
                char home = get_security_exchange(mm_home);
                add_single_char_val(t, fix_obj, "207", 3, home);
                if(getplen(doj, ROM_MM_ACC) > 0) {
                    set_fix_val(t, fix_obj, 440, doj->positions[ROM_MM_ACC].iov_base,
                            doj->positions[ROM_MM_ACC].iov_len);
                }
                break;
        }
}
void phlx_handle_parent_fill(dart_order_obj* orig,
...
void phlx_calc_leg_ratio(dart_order_obj* child, dart_order_obj* parent)
{
    char copier[32];
    memset(copier, '\0', 32);
    memcpy(&copier, parent->positions[ROM_SHARES].iov_base, parent->positions[ROM_SHARES].iov_len);
...
void reset_shares_on_replace(dart_order_obj* doj)
{
    ...
    phlx_calc_leg_ratio(ldoj, doj);
}
static void phlx_reset_fields_for_parent(dart_order_obj* orig,
                                    dart_order_obj* doj,
                                    con_interface* ci,
                                    void (*next_step) (struct connection_interface *,
                                           dart_order_obj * doj))
    ...
    if(getplen(orig, ROM_LEG_MARK) > 0 && mark[0] == '1') {
        ...
        if(found && lock_n_check(parent)) {
            ...
            if(status_len == 1 && (status[0] == '1' || status[0] == '2')) {
                phlx_handle_parent_fill(orig, parent, ci, next_step, status, status_len);
            } else {
                next_step(ci, parent);
                unlock(parent);
            }
    ...
    while(lch != NULL) {
        ...
        if(new) {
            ...
            int cur_size = create_in_place_ord_token(ci->tg,
                                                    leg_id, 12);
            char* last_five = leg_id;
            if(cur_size > 5) {
                last_five += (cur_size - 5);/*We know that the token will be at least 7 char long.*/
                cur_size = 5;
            }
            set_rom_field(ldoj, ROM_CLORDID, last_five,
                            cur_size);
            pthread_spin_lock(&ci->mutex);
            no_copy_insert(ci->cl_ord_id_map, last_five, cur_size,
                          ldoj);
...
static void set_clr_acc(trans_t* t, ofp* fix_obj, dart_order_obj* doj)
{
    char* clear = getpval(doj, ROM_CLR_ACC);
    int clen = getplen(doj, ROM_CLR_ACC);
    if(clen == 8 && strncmp(clear + 4, "1209", 4) == 0) {
        set_fix_val(t, fix_obj, 1, clear, 4);
    } else {
        set_fix_val(t, fix_obj, 1, clear, clen);
    }
}

static void set_phlx_ord_id(con_interface* ci, dart_order_obj* doj, int rom_field)
{
    char ord_id[16];
    memset(ord_id, '\0', 16);
    int cur_size = create_multiday_in_place_ord_token(ci->tg,
                                                ord_id, 12);
    fill_in_rom_field(doj, rom_field, ord_id,
                    cur_size);
}

int on_phlx_enter_order(con_interface * ci, dart_order_obj * doj,
        struct async_parse_args *ap)
{
    ...
    set_phlx_ord_id(ci, doj, ROM_CLORDID);
    pthread_spin_lock(&ci->mutex);
    ...
    if(doj->childs == 0) {
        char nsec [8];
        memset(nsec, '\0', 8);
        int cur_size = trans_sec_type(getpval(doj, ROM_SEC_TYPE), getplen(doj, ROM_SEC_TYPE),
                                  nsec);
        fill_in_rom_field(doj, ROM_EXEC_SEC_TYPE, nsec, cur_size);
        set_fix_val(t, fix_obj, 167, nsec, cur_size);
    ...
    if(getplen(doj, ROM_CAP) > 0) {
        ...
        trans_phlx_cap(t, fix_obj, getpval(doj, ROM_CAP), mmhome, mmlen, doj);
    }
...
int on_phlx_cancel_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
    ...
    set_phlx_ord_id(ci, doj, ROM_PENDING_ID);
...
void on_phlx_replace_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
    ofp* fix_obj = 0;
    if(doj->childs == 0) {
...
static ff_desc* create_phlx_ff_out_array(int* len, char* dir, int d_len)
{
...
static ff_desc* create_phlx_ff_in_array(int* len, char* dir, int d_len)
{
        ...
        gatekeeper * gk)
{
    ...
    reset_con_interface(con, 8192,
            ...
            send_fix_logon, build_phlx_offsets,
            on_phlx_enter_order, on_phlx_cancel_order,
            on_phlx_replace_order, gk, (char *) ex->dest_id,
            ex->dest_id_len);
    con->client_mess_parse = parse_fix_message;
    con->ci_parse_v = 0;
    con->on_connect = on_connect;
    con->ci_obj_callback= from_phlx;
    ...
    int out_len = 0;
    int in_len = 0;
    ff_desc* out_types = create_phlx_ff_out_array(&out_len,directory, ret_len);
    ff_desc* in_types = create_phlx_ff_in_array(&in_len,directory, ret_len);
    ...
    if(ret_len <=0 ) {
        con->sbm = create_service_bureau_matcher("phlx_clearing.map");
    } else {
    ...
    if (found && lock_n_check(orig)) {
        ...
        switch(type[0]) {
            case '0':
                ...
                if(is_spread(orig)) {
                    phlx_reset_fields_for_parent(orig, doj, ci, fix_return);
                }
                fix_return(ci, orig);
                break;
            case '1':
            case '2':
                {
                    ...
                    if(is_spread(orig)) {
                        phlx_reset_fields_for_parent(orig, doj, ci, ci->on_fill);
                    }
                    ci->on_fill(ci, orig);
                }
                break;
            ...
            case '4':
                ...
                if(is_spread(orig)) {
                    phlx_reset_fields_for_parent(orig, doj, ci, ci->on_out);
                }
                ci->on_out(ci, orig);
                if(!is_spread(orig)) {
                    destroy_orig = 1;
            ...
            case '5':
                ...
                if(is_spread(orig)) {
                    phlx_reset_fields_for_parent(orig, doj, ci, ci->on_replaced);
                }
            ...
            case '8':
                ...
                if(is_spread(orig)) {
                    phlx_reset_fields_for_parent(orig, doj, ci, ci->on_reject);
                }
