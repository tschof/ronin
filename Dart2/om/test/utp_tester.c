...
int test_utp_trade(const char **testname, const char **extrareporting)
{
    ...
    REQCOND(buff != 0);
    memset(rom_trade_checker->buffer, '3', 200);
    memset(buff->buffer, 0, 300);
    char* bck = rom_trade_checker->buffer;
    char* ack = buff->buffer;
    int i = 0;
    for(; i < 300; ++i) {
        char liq = '0';
        if(i % 2) {
...
int test_utp_time(const char **testname, const char **extrareporting)
{
    ...
    localtime_r(&rawtime, &day);
    char* mlbk_hour = day.tm_isdst ? "5" : "6";
    char* five_hour = day.tm_isdst ? "09" : "10";
    ti = &lm;
    set_date_str(buff->buffer, one_oclock);
    set_date_str(fbuff->buffer, five_oclock);
    sprintf(mlbk, "%04d%02d%02d-0%s:00:00", ti->tm_year + 1900,
            ti->tm_mon + 1, ti->tm_mday, mlbk_hour);
    sprintf(fiver, "%04d%02d%02d-%s:00:00", ti->tm_year + 1900,
            ti->tm_mon + 1, ti->tm_mday, five_hour);
...
int test_utp_price_trans(const char **testname,
                         const char **extrareporting)
{
    STARTTEST;
    databuf_t *buff = databuf_alloc(100);
    REQCOND(buff != 0);
