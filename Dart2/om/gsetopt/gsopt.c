...
void from_gsopt(dart_order_obj * doj, void *a);

int build_gsopt_offsets(struct async_parse_args *apa)
{
    //con_interface* con = (con_interface*)apa->ds_book;
    return 0;
}
static void trans_gsopt_cap(trans_t* t, ofp* fix_obj, char* cap,
...
void gsopt_reset_shares_on_replace(dart_order_obj* doj)
{
    ...
    gsopt_calc_leg_ratio(ldoj, doj);
}
static void gsopt_reset_fields_for_parent(dart_order_obj* orig,
        ...
        if(found && lock_n_check(parent)) {
            ...
            if(status[0] == '5') {
                ...
                if(rep != 0) {
                    ...
                    gsopt_reset_shares_on_replace(parent);
                }
            }
            if(status_len == 1 && (status[0] == '1' || status[0] == '2')) {
                gsopt_handle_parent_fill(orig, parent, ci, next_step, status, status_len);
            } else {
                next_step(ci, parent);
                unlock(parent);
            }
...
int on_gsopt_enter_order(con_interface * ci, dart_order_obj * doj,
        struct async_parse_args *ap)
{
    ...
    if(getplen(doj, ROM_CAP) > 0) {
        ...
        trans_gsopt_cap(t, fix_obj, getpval(doj, ROM_CAP), mmhome, mmlen);
    }
...
int on_gsopt_cancel_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
...
void on_gsopt_replace_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
    ofp* fix_obj = 0;
...
static ff_desc* create_gsopt_ff_out_array(int* len, char* dir, int d_len)
{
...
static ff_desc* create_gsopt_ff_in_array(int* len, char* dir, int d_len)
{
        ...
        gatekeeper * gk)
{
    ...
    reset_con_interface(con, 8192,
            ...
            send_fix_logon, build_gsopt_offsets,
            on_gsopt_enter_order, on_gsopt_cancel_order,
            on_gsopt_replace_order, gk, (char *) ex->dest_id,
            ex->dest_id_len);
    con->client_mess_parse = parse_fix_message;
    con->ci_parse_v = 0;
    con->on_connect = on_connect;
    con->ci_obj_callback= from_gsopt;
    ...
    int out_len = 0;
    int in_len = 0;
    ff_desc* out_types = create_gsopt_ff_out_array(&out_len,directory, ret_len);
    ff_desc* in_types = create_gsopt_ff_in_array(&in_len,directory, ret_len);
    ...
    if (found && lock_n_check(orig)) {
        ...
        switch(type[0]) {
            case '0':
                ...
                if(is_spread(orig)) {
                    gsopt_reset_fields_for_parent(orig, doj, ci, fix_return);
                }
                fix_return(ci, orig);
                break;
            case '1':
            case '2':
                {
                    ...
                    if(is_spread(orig)) {
                        gsopt_reset_fields_for_parent(orig, doj, ci, ci->on_fill);
                    }
                    ci->on_fill(ci, orig);
                }
                break;
            ...
            case '4':
                ...
                if(is_spread(orig)) {
                    gsopt_reset_fields_for_parent(orig, doj, ci, ci->on_out);
                }
                ci->on_out(ci, orig);
                if(!is_spread(orig)) {
                    destroy_orig = 1;
            ...
            case '5':
                ...
                if(is_spread(orig)) {
                    gsopt_reset_fields_for_parent(orig, doj, ci, ci->on_replaced);
                }
            ...
            case '8':
                ...
                destroy_gk_db(ci->gk, db);
                if(is_spread(orig)) {
                    gsopt_reset_fields_for_parent(orig, doj, ci, ci->on_reject);
                }
