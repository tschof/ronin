...
void get_rej_text(short reason, dart_order_obj * doj)
{
    ...
    case 3002:
        ...
        break;
    case 3003:
        reset_rom_field(doj, ROM_TEXT, "Target System unavailable", 25);
        break;
    case 3004:
        reset_rom_field(doj, ROM_TEXT, "Symbol is not a valid NYSE symbol",
                        33);
        break;
    case 4000:
        reset_rom_field(doj, ROM_TEXT, "NYSE down or stock halted", 25);
        break;
    case 4001:
        reset_rom_field(doj, ROM_TEXT, "Invalid stock symbol", 20);
        break;
    case 2:
        reset_rom_field(doj, ROM_TEXT, "Symbol already open", 19);
    }
}
                             ...
                             con_interface * ob)
{
    ...
    *buff->wr_ptr++ = 'A';
    ...
    cur_size = in_place_order_id(((nyse_seq_struct*)ob->parser), rph->current->data->wr_ptr);
    reset_rom_field(rph, ROM_CLORDID, rph->current->data->wr_ptr,
                    cur_size);
    reset_rom_field(rph, ROM_EX_TAG, rph->current->data->wr_ptr,
                    cur_size);
                          ...
                          con_interface * ob)
{
    ...
    *buff->wr_ptr++ = off[0];
    ...
    if(getplen(ute, ROM_PENDING_ID) > 0) {
        cur_size = in_place_order_id(((nyse_seq_struct*)ob->parser),
                           ...
                           con_interface * ob)
{
    ...
    *buff->wr_ptr++ = 'A';
    ...
    if(getplen(ute, ROM_PENDING_ID) > 0) {
        ...
        check_and_resize(ute, 17);
        cur_size = in_place_order_id(((nyse_seq_struct*)ob->parser), ute->current->data->wr_ptr);
        reset_rom_field(ute, ROM_PENDING_ID, ute->current->data->wr_ptr,
                        cur_size);
