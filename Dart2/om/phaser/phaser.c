...
void from_phaser(dart_order_obj * doj, void *a);


int build_phaser_offsets(struct async_parse_args *apa)
{
...
int on_phaser_enter_order(con_interface * ci, dart_order_obj * doj,
        struct async_parse_args *ap)
{
    ...
    if(getplen(doj, ROM_MM_ACC) > 0) {
        set_fix_val(t, fix_obj, 440, getpval(doj,ROM_MM_ACC), getplen(doj,ROM_MM_ACC));
    }
    ...
    if(getplen(doj, ROM_COPY_INSTR) == 2) {
        char* instr = getpval(doj, ROM_COPY_INSTR);
        if(instr[0] == '9') {
            set_fix_val(t, fix_obj, 7411, "9", 1);
            set_fix_val(t, fix_obj, 7472, getpval(doj,ROM_COPY_INSTR), getplen(doj,ROM_COPY_INSTR));
        }
    }
    if(doj->childs != 0) {
...
int on_phaser_cancel_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
...
void on_phaser_replace_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
    ofp* fix_obj = 0;
    if(doj->childs == 0) {
    ...
    if(getplen(doj, ROM_INSTR) == 2) {
        char* instr = getpval(doj, ROM_INSTR);
        if(instr[0] == '9') {
            set_fix_val(t, fix_obj, 7411, "9", 1);
            set_fix_val(t, fix_obj, 7472, getpval(doj,ROM_INSTR), getplen(doj,ROM_INSTR));
        }
    }
    if(doj->childs != 0) {
        add_spread_legs(t, doj, fix_obj, ci, 0);
...
static void phaser_on_connect(struct connection_interface *ci, int is_connected)
{
    on_connect(ci, is_connected);
}
static ff_desc* create_phaser_ff_out_array(int* len, char* dir, int d_len)
{
...
static ff_desc* create_phaser_ff_in_array(int* len, char* dir, int d_len)
{
        ...
        gatekeeper * gk)
{
    ...
    reset_con_interface(con, 8192,
            ...
            send_fix_logon, build_phaser_offsets,
            on_phaser_enter_order, on_phaser_cancel_order,
            on_phaser_replace_order, gk, (char *) ex->dest_id,
            ex->dest_id_len);
    con->client_mess_parse = parse_fix_message;
    con->ci_parse_v = 0;
    con->on_connect = phaser_on_connect;
    con->ci_obj_callback= from_phaser;
    ...
    int out_len = 0;
    int in_len = 0;
    ff_desc* out_types = create_phaser_ff_out_array(&out_len,directory, ret_len);
    ff_desc* in_types = create_phaser_ff_in_array(&in_len,directory, ret_len);
        ...
        dart_order_obj* obj)
{
    ...
    if(getplen(obj, FIX_EXE_TYPE) > 0) {
        ...
        if(t[0] == '1') {
            memcpy(title, "Bust from PHASER", 13);
            t_len = 13;
        } else {
            memcpy(title, "Correction from PHASER", 19);
            t_len = 19;
        }
    } else {
        memcpy(title, "Unmatched Fill from PHASER", 23);
        t_len = 23;
    }
    ...
    if(getplen(obj, FIX_EXEC_PRICE) > 0) {
        databuf_memcpy(cur_off, "Last Prphaser: ", 12);
    ...
    if (found && lock_n_check(orig)) {
        ...
        switch(type[0]) {
            case '0':
                ...
                if(is_spread(orig)) {
                    reset_fields_for_parent(orig, doj, ci, fix_return);
                }
                fix_return(ci, orig);
                break;
            case '1':
            ...
            case '8':
                ...
                if(is_spread(orig)) {
                    if(getplen(orig, ROM_OM_PARENT) > 0) {
                        reset_fields_for_parent(orig, doj, ci, ci->on_reject);
                    } else {
