...
void from_citadel(dart_order_obj * doj, void *a);


int build_citadel_offsets(struct async_parse_args *apa)
{
...
int on_citadel_enter_order(con_interface * ci, dart_order_obj * doj,
        struct async_parse_args *ap)
{
   ...
   // doj->current->data->wr_ptr += cur_size;
    ...
    if(getplen(doj, ROM_ALGO_TYPE) > 0) {
        ...
        if(algo[0] == '1') {
            set_fix_val(t, fix_obj, 847, "1003",4);
        } else {
            set_fix_val(t, fix_obj, 847, "1001",4);
        }
    }
...
int on_citadel_cancel_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
...
void on_citadel_replace_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
    ofp* fix_obj = 0;
...
static void citadel_on_connect(struct connection_interface *ci, int is_connected)
{
    on_connect(ci, is_connected);
}
static ff_desc* create_citadel_ff_out_array(int* len, char* dir, int d_len)
{
...
static ff_desc* create_citadel_ff_in_array(int* len, char* dir, int d_len)
{
        ...
        gatekeeper * gk)
{
    ...
    reset_con_interface(con, 8192,
            ...
            send_fix_logon, build_citadel_offsets,
            on_citadel_enter_order, on_citadel_cancel_order,
            on_citadel_replace_order, gk, (char *) ex->dest_id,
            ex->dest_id_len);
    con->client_mess_parse = parse_fix_message;
    con->ci_parse_v = 0;
    con->on_connect = citadel_on_connect;
    con->ci_obj_callback= from_citadel;
    ...
    int out_len = 0;
    int in_len = 0;
    ff_desc* out_types = create_citadel_ff_out_array(&out_len,directory, ret_len);
    ff_desc* in_types = create_citadel_ff_in_array(&in_len,directory, ret_len);
