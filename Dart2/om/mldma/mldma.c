...
void from_mldma(dart_order_obj * doj, void *a);


int build_mldma_offsets(struct async_parse_args *apa)
{
...
static void set_sym(trans_t* t, ofp* fix_obj, dart_order_obj* doj)
{
    ...
    if(slash != NULL) {
        ...
        set_fix_val(t, fix_obj, 48, getpval(doj, ROM_SYM), getplen(doj, ROM_SYM));
    } else {
        set_fix_val(t, fix_obj, 48, getpval(doj, ROM_SYM), getplen(doj, ROM_SYM));
...
int on_mldma_enter_order(con_interface * ci, dart_order_obj * doj,
...
int on_mldma_cancel_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
...
void on_mldma_replace_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
    ofp* fix_obj = 0;
    ...
    if(getplen(rph, ROM_EXPIRE_TIME) > 0) {
        set_fix_val(t, fix_obj, 126, getpval(rph,ROM_EXPIRE_TIME), getplen(rph,ROM_EXPIRE_TIME));
    ...
    } else if(getplen(doj, ROM_MAX_FLOOR) > 0) {
        set_fix_val(t, fix_obj, 111, getpval(doj,ROM_MAX_FLOOR), getplen(doj,ROM_MAX_FLOOR));
    }
    if(getplen(doj, ROM_SHORT_LEND) > 0) {
        set_fix_val(t, fix_obj, 114, "Y", 1);
...
static void mldma_on_connect(struct connection_interface *ci, int is_connected)
{
    on_connect(ci, is_connected);
}
static ff_desc* create_mldma_ff_out_array(int* len, char* dir, int d_len)
{
...
static ff_desc* create_mldma_ff_in_array(int* len, char* dir, int d_len)
{
        ...
        gatekeeper * gk)
{
    ...
    reset_con_interface(con, 8192,
            ...
            send_fix_logon, build_mldma_offsets,
            on_mldma_enter_order, on_mldma_cancel_order,
            on_mldma_replace_order, gk, (char *) ex->dest_id,
            ex->dest_id_len);
    con->client_mess_parse = parse_fix_message;
    con->ci_parse_v = 0;
    con->on_connect = mldma_on_connect;
    con->ci_obj_callback= from_mldma;
    ...
    int out_len = 0;
    int in_len = 0;
    ff_desc* out_types = create_mldma_ff_out_array(&out_len,directory, ret_len);
    ff_desc* in_types = create_mldma_ff_in_array(&in_len,directory, ret_len);
        ...
        dart_order_obj* obj)
{
    ...
    if(getplen(obj, FIX_EXE_TYPE) > 0) {
        ...
        if(t[0] == '1') {
            t_len = sprintf(title, "Bust from 2SIGMA");
        } else {
            t_len = sprintf(title, "Correction from 2SIGMA");
        }
    } else {
        t_len = sprintf(title, "Unmatched Fill from 2SIGMA");
    }
            ...
            case '8':
                ...
                if(getplen(doj, FIX_TEXT) > 0) {
                    ...
                    clean_text(orig, out_mess, 
                                    ROM_TEXT, doj->positions[FIX_TEXT].iov_base,
                                    doj->positions[FIX_TEXT].iov_len);
