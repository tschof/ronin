...
void from_nfx(dart_order_obj * doj, void *a);


int build_nfx_offsets(struct async_parse_args *apa)
{
    //con_interface* con = (con_interface*)apa->ds_book;
    return 0;
}

static void trans_nfx_cap(trans_t* t, ofp* fix_obj, char* cap)
{
        switch(cap[0]) {
            case 'A':
                set_fix_val(t, fix_obj, 528, "A", 1);
                break;
            case 'P':
                set_fix_val(t, fix_obj, 528, "P", 1);
                break;
            case 'X':
                set_fix_val(t, fix_obj, 582, "1", 1);
                break;
        }
}
...
int on_nfx_enter_order(con_interface * ci, dart_order_obj * doj,
        struct async_parse_args *ap)
{
    ...
    if(getplen(doj, ROM_EXPIRE_TIME) > 0) {
        set_fix_val(t, fix_obj, 432, getpval(doj,ROM_EXPIRE_DATE), getplen(doj,ROM_EXPIRE_DATE));
    }
    if(getplen(doj, ROM_CAP) > 0) {
        trans_nfx_cap(t, fix_obj, getpval(doj, ROM_CAP));
    }
    if(doj->childs != 0) {
...
int on_nfx_cancel_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
...
void on_nfx_replace_order(con_interface * ci, dart_order_obj * doj)
{
    trans_t* t = (trans_t*)ci->parser;
    ofp* fix_obj = 0;
    if(doj->childs == 0) {
...
static void nfx_on_connect(struct connection_interface *ci, int is_connected)
{
        ...
        gatekeeper * gk)
{
    ...
    reset_con_interface(con, 8192,
            ...
            send_fix_logon, build_nfx_offsets,
            on_nfx_enter_order, on_nfx_cancel_order,
            on_nfx_replace_order, gk, (char *) ex->dest_id,
            ex->dest_id_len);
    con->client_mess_parse = parse_fix_message;
    con->ci_parse_v = 0;
    con->on_connect = nfx_on_connect;
    con->ci_obj_callback= from_nfx;
    ...
    int out_len = 0;
    int in_len = 0;
    ff_desc* out_types = create_nfx_ff_out_array(&out_len,directory, ret_len);
    ff_desc* in_types = create_nfx_ff_in_array(&in_len,directory, ret_len);
