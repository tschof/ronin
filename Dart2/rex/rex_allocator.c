...
static void create_keys(dallocator* dmg, wtr* cur_glob)
{
    wtr* keyglob = 0;
    int ret = 0;
    REX_ALLOC(keyglob,8, sizeof(struct walker_texas_ranger), ret);
    keyglob->payload = 0;
    REX_ALLOC(keyglob->payload, REX_PG_SIZE, REX_PG_SIZE, ret);
...
static void set_ready_list(dallocator* dmg, wtr* cur_glob)
{
    ...
    if(dmg->size < REX_PG_SIZE) {
        total = REX_PG_SIZE / dmg->size;
        cur_glob->payload = 0;
        REX_ALLOC(cur_glob->payload, REX_PG_SIZE,REX_PG_SIZE, ret);
    } else {
        cur_glob->payload = 0;
        REX_ALLOC(cur_glob->payload, REX_PG_SIZE, dmg->size, ret);
        total = dmg->size / REX_PG_SIZE;
        step = REX_PG_SIZE;
    }
...
dallocator* create_mem_glob(unsigned int size,void init_func(void *))
{
    int ret = 0;
    dallocator* dmg = 0;
    REX_ALLOC(dmg, 8, sizeof(struct rex_mem_glob), ret);
    dmg->empty = 0;
    dmg->ready = 0;
    dmg->globs = 0;
    REX_ALLOC(dmg->globs, 8, sizeof(struct walker_texas_ranger), ret);
    dmg->globs->next = 0;
    dmg->size = size;
...
void* get_stuff(dallocator* dmg)
{
    ...
    if(dmg->ready == 0) {
        wtr* glob1 = 0;
        int ret = 0;
        REX_ALLOC(glob1, 8,sizeof(struct walker_texas_ranger), ret);
