...
uint32_t murmerhash(const unsigned char* str, int str_len, uint32_t map_size)
{
    uint64_t h = 0x0;
    memcpy(&h, str, str_len);
    h ^= h >> 33;
...
uint32_t hash6432(const unsigned char* str, int str_len, uint32_t map_size)
{
    uint64_t h = 0x0;
    memcpy(&h, str, str_len);
    h = (h << 18) - h - 1;
