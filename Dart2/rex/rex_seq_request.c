                         ...
                         short reset_min, time_t last_logon, time_t now)
{
    ...
    ti = &lm;
    localtime_r(&last_logon, &ll);
    int day_offset = reset_day_offsets[ti->tm_wday][ll.tm_wday];
    if(reset_day == 7 && 
            (day_offset > 1 && day_offset < 7)) {
        create_new = 1;
    } else if(reset_day == 7 || reset_day == ti->tm_wday) {
        int sec_diff = ((lm.tm_hour * 3600) + (lm.tm_min * 60))  -
             ((reset_hour * 3600) + (reset_min * 60));
        time_t reset_time = now - sec_diff;
        create_new = (now >= reset_time && last_logon < reset_time) ? 1 : 0;
    } else if(reset_day != 7) {
        int reset_offset = reset_day_offsets[ti->tm_wday][reset_day];        
        struct tm t;
        memset(&t, 0, sizeof(struct tm));
        t.tm_year = ti->tm_year;
        t.tm_mon = ti->tm_mon;
        t.tm_mday = ti->tm_mday - reset_offset;
        t.tm_hour = reset_hour;
        t.tm_min = reset_min;
        t.tm_sec = 0;
        t.tm_isdst = -1;
...
{
    sequence_req *sr = malloc(sizeof(struct rex_seq_request));
    if (sr != NULL) {
        time_t now;
        time(&now);
        ...
        sr->create_new = ex->create_new;
        sr->new_incoming = ex->new_incoming;
        sr->new_outgoing = ex->new_outgoing;
        sr->incoming = 0;
        sr->outgoing = 0;
        sr->parse_messages = p_messages;
        sr->fix_version = version; //ex->version;
        sr->create_reset_pad = create_pad;
        sr->p_helper = p_helper;
        if(ex->create_new == 0 && ex->last_logon != 0) {
            sr->create_new = ll_before_reset_time(ex->reset_day, ex->reset_hour,
                                                  ex->reset_min, ex->last_logon, now);
        }
        ex->last_logon = now;
