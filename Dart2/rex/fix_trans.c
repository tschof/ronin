...
static char* fix_get_file_contents(const char* filename, long* size)
{
    FILE* mfile = 0;
    long res = 0;
    char* buff = 0;
...
static void fix_build_map_from_file(const char* filename, hashmap_t map)
{
    char* buffer = 0;
    long size = 0;
    buffer = fix_get_file_contents(filename, &size);
    if(size > 0) {
        char* start = buffer;
        char* comma = buffer;
        char* end = buffer;
...
{
    char* buff = fix_get_file_contents(filename, ret_size);
    if(*ret_size > 0) {
        long size = *ret_size;
        char* off = buff;
        do {
...
static void create_fix_incoming_trans(trans_t* t, char* filename, short type)
{
    ...
    fix_build_map_from_file(filename, l);
        ...
        ff_desc* in_types, int in_len)
{
    ...
    pthread_spin_init(&t->queue_mut, PTHREAD_PROCESS_PRIVATE);
        ...
        dart_order_obj* obj, short type)
{
    ...
    char* val = fix_msg;
    char* end = fix_msg;
    char* mend = fix_msg + len;
    int* field = 0;
    hashmap_t transy = 0;
    transy = rb_tree_get_val(t->in_types, &type);
    if(transy) {
        do {
            end = memchr(tag, '=', mend - tag);
            if(end) {
                val = end;
                end = memchr(end, FIX_FIELD_DELIM, mend - end);
                if(end) {
                    int found = find_n_fill(transy, tag, (val - tag),
                                (void **)&field);
                    if(found > 0) {
                        ++val;
                        reset_rom_field(obj, *field, val, end - val);
                    } 
                    ++end;
                    tag = end;
                }
...
void trans_n_copy(trans_t* t, char* fix_msg, int len,
                  dart_order_obj* obj, short type)
{
    int* field = 0;
    char* tag = fix_msg;
    ...
    if(transy) {
        do {
            if(*end == '=') {
                ...
                if(found > 0) {
                    ++val;
                    set_rom_field(obj, *field, val, end - val);
                }
                tag = end + 1;
            }
            ++end;
...
{
    ofp* doj = 0;
    pthread_spin_lock(&t->queue_mut);
    dallocator* p = rb_tree_get_val(t->out_types, &type);
    if(p) {
        doj = get_stuff(p);
    }
    pthread_spin_unlock(&t->queue_mut);
    return doj;
}
...
void set_fix_val(ofp* obj, int offset, char* val, int val_len)
{
...
void set_char_fix_val(ofp* obj, char* fix_tag, int f_len, char* val, int val_len)
{
...
void add_single_char_val(ofp* obj, char* fix_tag, int f_len, char val)
{
    databuf_memcpy(obj->buff, fix_tag, f_len);
    *obj->buff->wr_ptr++ = '=';
    *obj->buff->wr_ptr++ = val;
...
int overwrite_fix_val(ofp* p, int offset, char* val, int val_len,
                      int indent)
{
    int ret_val = 0;
    ...
    if(off >= 0) {
        ...
        set_fix_val(p, offset, val, val_len);
    }
    return ret_val;
}
void set_tag_nine(ofp* obj)
{
...
void set_num_val(ofp* obj, int fix_tag, unsigned int value)
{
...
void add_num_val(ofp* obj, int fix_tag, unsigned int value)
{
    unsigned int len = local_unsigned_itoa(obj->buff->wr_ptr, fix_tag);
    obj->buff->wr_ptr += len;
    *obj->buff->wr_ptr++ = '=';
...
void add_value(ofp* obj, char* fix_tag, int len, unsigned int val)
{
...
void safe_num_val(ofp* obj, int fix_tag, unsigned int value)
{
    databuf_write(obj->buff, "%d=%ld%c", fix_tag, value, '\x01');
}
void safe_val_add(ofp* obj, const char* fix_tag, int flen,
                  char* val, int len)
{
