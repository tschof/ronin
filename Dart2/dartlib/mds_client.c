...
static int mds_send_wrapper(int sock, char* data, int len, int* disconnect, otl* o)
{
    int sent = 0;
    int ret = 0;
    do {
        ...
        if(ret < 0) {
            ...
            if(ret == EAGAIN || ret == EWOULDBLOCK || ret == EINTR ||
                ...
                send_debug_message(o, "Got this error on send: %d, killing con \n",
                        ret);
                ret = -1;
                *disconnect = 1;
            }
...
static int send_mdc_logon(mdsclient* mdc)
{
    ...
    memset(b, '\0', 32);
    int len = 0;
    char* user = mdc->ex->username;
    int ulen = mdc->ex->ulen;
    len = ulen + 1;
    memcpy(b, &len, 4);
    memcpy(b + 4, "L", 1);
    memcpy(b + 5, user, ulen);
    int dis = 0;
    return mds_send_wrapper(mdc->sock, b, len + 4, &dis, mdc->o);
}
static int send_mdc_hb(mdsclient* mdc)
{
    ...
    memcpy(b + 5, user, ulen);
    int dis = 0;
    mds_send_wrapper(mdc->sock, b, len + 4, &dis, mdc->o);
    if(dis == 1) {
        return 0;
    } else {
        return 1;
...
static void connect_mds(mdsclient* mdc)
{
    ...
    mdc->sock = create_mds_con(mdc, mdc->ex->port, mdc->ex->ip, &s);
    while(mdc->sock == -1) {
    ...
    while(dlen > 4) { 
        ...
        if((len + 4) <= dlen) {
            if(data[4] == 'S') {
                ...
                send_debug_message(mdc->o, "Got a heartbeat: %d \n", len);
            }
            len += 4;
            data += len;
            dlen -= len;
            ...
            if(len > 64) {
                send_debug_message(mdc->o, "Got a large len: %d \n", len);
            }
            break;
        }
    }
...
static void recv_loop(mdsclient* mdc)
{
    int ret = 0;
    char* buffer = 0;
    REX_ALLOC(buffer, 8,MDS_BUFFER_SIZE + 1, ret);
    char* wr_ptr = buffer;
    char* rd_ptr = buffer;
    ...
    do {
        ...
        if(ret == -1) {
            ...
            if (e != EAGAIN || e != EWOULDBLOCK) {
                send_debug_message(mdc->o, "MDS Error on select: %d \n", e);
                goto LOG_DONE;
            }
        } else if(ret == 0) {
            ...
            if (num_bytes > 0) {
                ...
                if (rd_ptr == wr_ptr ||
                    ...
                    if(end - wr_ptr <= 0 ) {
                        ...
                        send_debug_message(mdc->o, "MDS Reallocing total: %ld , unread: %ld \n",
                                total, write_offset);
                ...
                int e = errno;
                if (e != EAGAIN || e != EWOULDBLOCK) {
                    send_debug_message(mdc->o, "MDS Error on read: %d \n", e);
                    goto LOG_DONE;
                }
            }  else {
                goto LOG_DONE;
...
static void* run_mds(void* s)
{
    ...
    if(mdc->cpu >= 0) {
        ...
        if (s != 0) {
            if (s == EFAULT) {
                send_debug_message
                    (mdc->o, "Invalid mem addr: %d Thread: %d, cpu: %d.\n", s,
                     1, mdc->cpu);
            } else if (s == EINVAL) {
                send_debug_message
                    (mdc->o, "Bad processor or cpu: %d Thread: %d, cpu: %d.\n", s,
                     1, mdc->cpu);
            } else if (s == ESRCH) {
                send_debug_message(mdc->o, "Bad Thread: %d Thread: %d, cpu: %d.\n",
                                   s, 1, mdc->cpu);
            } else {
                send_debug_message
                    (mdc->o, "Unknown error: %d Thread: %d, cpu: %d.\n", s,
                     1, mdc->cpu);
            }
        } else {
            send_debug_message(mdc->o, "MDS Thread number: %d, cpu affinity: %d.\n",
                               1, mdc->cpu);
        }
    }
    while(mdc->run) {
...
{ 
    mdsclient* mdc = 0;
    int ret = 0;
    REX_ALLOC(mdc, 8, sizeof(struct mds_client_con), ret);
    mdc->ex = ex;
    ...
    if(error) {
        send_debug_message(mdc->o, "FAILED to create MDS THREAD: %d, cpu affinity: %d.",
                           1, mdc->cpu);
        mdc->run = 0;
        return NULL;
    } else {
