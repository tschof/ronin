...
module Converters =
    ...
    type OutCat = JsonProvider< "../CATWeb1/data/IM 2.2.1.json">
    ...
    let generateCsvSchema = 
        let eventRoot = OutCat.GetSample()
        let fs = FilePackager.createCsvEventFile "Schemas"
        eventRoot.EventDefinitions |> Seq.iter(fun root -> 
                                    let v =root.EventName                                     
                                    let v1 = (root.Fields |> Array.map(fun f -> 
                                                                            match f.Required with 
                                                                            | "Required" -> sprintf "%s (string)," f.Name
                                                                            | _ -> sprintf "%s (string option)," f.Name)) |> Array.fold (+) ""
                                    fs.WriteLine ("type " + v + " = CsvProvider<Schema = \"" + v1 + " \",HasHeaders=false>"); fs.Flush()
                                    )
    ...
    let getExchange (key:string, destMap:Map<string,CatConfig.DestIMID>) =
        let cimid = Map.find key destMap
        cimid.exchangeID

    let getExType (key:string, destMap:Map<string, CatConfig.DestIMID>) =
        let cimid = Map.find key destMap
        cimid.desttype
    ...
    let utcToEastern (time:string) = 
        ...
        | x when x >= 21 -> 
                ...
                let estZone = System.TimeZoneInfo.FindSystemTimeZoneById("Eastern Standard Time")
    ...
    let createNewOrderEvent(imi:CatConfig.IMConfig,
                            csv:RomMessage.Row) =
        ...
        | Some custAccount ->  
                                let firmid = (match custAccount.FirmId.String with
                                             | None -> match custAccount.FirmId.Number with
                                                        | None -> raise(CatConfig.BadCatConfig("No Firm ID for account in create New Ord"))
                                                        | Some x -> x.ToString()
                                             | Some x -> x)
    ...
    let createOrderRouteRpt(imi:CatConfig.IMConfig,
                            csv:RomMessage.Row,
                            romTime:string) =
        ...
        let session = match dest.DestType with
                        ...
                        | _ -> None
        MEOR.Row("NEW", None, createRoeID(csv.Exchange, csv.Romtime), "MEOR", Some imi.imid, convertTime csv, orderId,
                  csv.Symbol, None, utcToEastern csv.Romtime, "false", "false",None, Some dest.Imid, 
    ...
    let createOrderAccepted(imi:CatConfig.IMConfig,
                            csv:RomMessage.Row) =
    ...
    let createOrderCancelledEvent(imi:CatConfig.IMConfig,
                                   csv:RomMessage.Row) =
        let dest = match imi.destinations.TryFind csv.Exchange with
                            |None -> raise(UnknownExchange(sprintf "%s%s"csv.Exchange csv.Romtag))
                            |Some x -> x
    ...
    let createOrderModifiedEvent(imi:CatConfig.IMConfig,
                                  csv:RomMessage.Row,
    ...
    let buildClientNewAndRouteCsv (rom:seq<RomMessage.Row>,
                                    imi:CatConfig.IMConfig) = 
        rom
        |> Seq.filter(fun elem -> elem.Command.Equals("E"))
        |> Seq.filter(fun elem -> match imi.custMap.TryFind elem.Clearingaccount with 
                                    | None -> false
                                    | Some _ -> true)
        |> Seq.map(fun elem -> (createNewOrderEvent(imi, elem), createOrderRouteRpt(imi, elem, utcToEastern(elem.Romtime))))
    ...
    let getOrigTimes(fquery:RetrieveCSVFileRecs.FFileQuery) =
        let orig = Map.empty
        RetrieveCSVFileRecs.RetrieveEquityDirRecords(fquery)
        |> Array.fold(fun (acc:Map<string,string>) elem -> RomMessage.Load(elem).Rows
                                                                    |> Seq.filter(fun elem -> elem.Symbol <> "ZVZZT" && elem.Symbol <> "NTEST")
                                                                    |> Seq.filter(fun elem -> elem.Command.Equals("E"))
                                                                    |> Seq.fold (fun  acc elem -> acc.Add(elem.Romtag, elem.Romtime)) acc) orig 
                                                                                                
        
    let buildEquityFiles(fquery:RetrieveCSVFileRecs.FFileQuery) =
        let fjunior = match fquery with
                        |RetrieveCSVFileRecs.FFileQuery(_, _, month, day) -> (day + month)
        let origTimes = getOrigTimes(fquery)
        let files = RetrieveCSVFileRecs.RetrieveEquityDirRecords(fquery)
        let imids = CatConfig.getIMIDS() |> List.ofArray
        for limid in imids do
            let ofile = FilePackager.createDataFile((sprintf "%s%s" limid.imid fjunior))
            files |> Array.iter(fun elem -> let rows = RomMessage.Load(elem).Rows
                                            let noTest = rows |> Seq.filter(fun elem -> 
                                                                                    elem.Symbol <> "ZVZZT" 
                                                                                    && elem.Symbol <> "NTEST")
                                                               |> Seq.filter(fun elem -> match limid.destinations.TryFind(elem.Exchange) with
                                                                                            | None -> printfn "%s%s" elem.Romtag elem.Exchange; false
                                                                                            | Some _ -> true)
                                            let custOrds = buildClientNewAndRouteCsv(noTest, limid)
                                            let bdAccepts = buildAccept(noTest, limid)
                                            let bdRoutes = buildRouteForBD(noTest, limid, origTimes)
                                            let replaces = buildClientReplaceNRoute(noTest, limid, origTimes)
                                            let cancels = buildClientCancel(noTest, limid)
                                            custOrds |> Seq.iter(fun elem -> match elem with
                                                                             | (None, x )-> let f = new MEOR([x]);
                                                                                            ofile.sw.Write(f.SaveToString());
                                                                                                       ofile.sw.Flush();
                                                                                                       ofile.recordCount <- ofile.recordCount + 1
                                                                             | (Some x, y) -> let f = new MENO([x]); 
                                                                                              let f2 = new MEOR([y]);
                                                                                              ofile.sw.Write(f.SaveToString());
                                                                                              ofile.sw.Write(f2.SaveToString());
                                                                                              ofile.sw.Flush();
                                            ...
                                            cancels |> Seq.iter(fun elem -> let f = new MEOC([elem]);
                                                                            ofile.sw.Write(f.SaveToString());
                                                                             ofile.sw.Flush();
                                                                             ofile.recordCount <- ofile.recordCount + 1))
            FilePackager.createMetaFile([ofile], (sprintf "%s%s" limid.imid fjunior))
