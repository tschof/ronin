...
int test_last_logon_logic(const char** testname, const char** extrareporting)
{
    ...
    char* buff = calloc(30, 1);
    REQCOND(buff != 0);
    login_info* in = calloc(1, sizeof(login_info));
    //year different and last logon day less than current day.
    memcpy(buff, "20130509220324", 14);
    in->last_logon = convert_to_sec_since_1970(buff);
    memcpy(buff, "20120210230425", 14);
    time_t rawtime = convert_to_sec_since_1970(buff);
    in->reset_hour = 23;
    in->reset_min = 9;
    in->reset_day = 7;
    int result = ll_before_reset_time(in->reset_day, in->reset_hour,
                                      in->reset_min, in->last_logon, rawtime);
    CHKCOND(result == 0);
    ...
    in->reset_hour = 0;
    in->reset_min = 0;
    result = ll_before_reset_time(in->reset_day, in->reset_hour,
                                      in->reset_min, in->last_logon, rawtime);
    CHKCOND(result == 1);
    //Hour is behind
