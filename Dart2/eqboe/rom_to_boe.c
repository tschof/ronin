                             ...
                             con_int* ob, token_gen* tg, uint64_t seq)
{
    ...
    set_rom_field(rph, ROM_BIN_QTY, &net_qty, sizeof(uint32_t));
    set_rom_field(rph, ROM_BIN_LEAVES, &net_qty, sizeof(uint32_t));
    uint32_t bcum = 0;
    set_rom_field(rph, ROM_BIN_CUM, &bcum, sizeof(uint32_t));
    ...
    if(mpid_len >= 7 && strncmp("CRON", clearing_account, 4) == 0) {
        ...
        int fclr_len = 0;
        char* bclr = (char*)get_suffix(bmaster->clr_accs, clearing_account, mpid_len,
                   &fclr_len);
    ...
    if(getplen(rph, ROM_SYM) > 0) {
        ...
        if(slash != NULL) {
            ...
            int suf_len = rph->positions[ROM_SYM].iov_len - (sym_len + 1);
            int fsufflen = 0;
            /*const char* bsuffix =  boe_get_suffix(b, (slash + 1), suf_len,
                       &fsufflen);*/
            const char* bsuffix = get_suffix(bmaster->suffix_map,
                    (slash + 1), suf_len,
                    &fsufflen);
    ...
    if(getplen(rph, ROM_CLR_ACC) > 0) {
        ...
        int fclr_len = 0;
        char* bclr = (char*)get_suffix(bmaster->clr_accs, clearing_account, acc_len,
                   &fclr_len);
...
int create_boe_replace_message(databuf_t * buff, rcntr *cnt,
                           con_int* ob, token_gen* tg, uint64_t seq)
{
    memset(buff->buffer, 0, 256);
    ...
    no_copy_insert(ob->cl_ord_id_map, getpval(rph,ROM_PENDING_ID),
            getplen(rph,ROM_PENDING_ID),cnt);
    ord->nrplbfields = 2;
    ...
    if(mpid_len >= 7 && strncmp("CRON", on_behalf, 4) == 0) {
        ...
        int fclr_len = 0;
        char* bclr = (char*)get_suffix(bmaster->clr_accs, on_behalf, mpid_len,
                   &fclr_len);
        if(fclr_len >= 7 && strncmp("74X5", bclr, 4) == 0) {
            int off = fclr_len > 7 ? 4 : 3;
            char* temp = bclr + off;
    ...
    if(rep->positions[ROM_SHARES].iov_len > 0 &&
        ...
        databuf_memcpy(buff, &cnt->size, 4);
    }
...
static void set_time(dart_order_obj* ute, char* mess) 
{
    long tt = 0;
    memcpy(&tt, (mess + 10), 8);
    char itoa_buff[64];
    ...
    if(rep && is_present) {
        if(rep->positions[ROM_SHARES].iov_len > 0) {
            ...
            if(ute->positions[ROM_BIN_QTY].iov_len > 0) {
                ...
                set_rom_field(ute, ROM_BIN_QTY, &l_q, sizeof(int)); 
            }
            set_rom_field(ute, ROM_SHARES, rep->positions[ROM_SHARES].iov_base, 
                                rep->positions[ROM_SHARES].iov_len); 
            ...
            } else if(ute->positions[ROM_CUM].iov_len > 0) {
                l_cum = strtol(rep->positions[ROM_CUM].iov_base, NULL, 10);
                set_rom_field(ute, ROM_BIN_CUM, &l_cum, sizeof(int)); 
            } 
...
int boe_parse_filled(rcntr* cnt, char *ack, con_int * ob)
{
    ...
    cnt->cum = cnt->cum + last_shares;
    cnt->last = last_shares;
    if(cnt->size <= cnt->cum || leaves_shares == 0) {
        set_rom_field(ute, ROM_STATUS, "2", 1);
        ret_val = 1;
    } else {
        set_rom_field(ute, ROM_STATUS, "1", 1);
    }
    dart_itoa(ute, cnt->cum, ROM_CUM, buff);
    databuf_reset(buff);
