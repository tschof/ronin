...
static void nb_direct_send(con_int* ci, dnbparse* up, void* data, int len)
{
    int bytes_sent = 0;
...
static int nb_sender(dnbparse* up, databuf_t* buff)
{
    int bytes_sent = 0;
    if(pthread_spin_lock(&up->send_mutex) == 0) {
        int disconnect = 0;
...
void nb_write_callback(void* cb)
{
    rex_con* rc = (rex_con*)cb;
    dnbparse* up = (dnbparse*)rc->parser;
    int blen = 0;
    int disconnect = 0;
...
static void nb_client_connect(void* pass_back, int is_connected,
        dart_order_obj* log)
{
    dnbparse* uparse = (dnbparse*)pass_back;
    ...
    send_debug_message(ci->o, "About to call on connect: %d \n", is_connected);
    uparse->dnbc->on_connect(is_connected);
    send_debug_message(ci->o, "Just Called on connect: %d \n", is_connected);
}
...
{
    if(dnbp->nyse->connected) {
        int cut_connection = 0;
        manage_socket(dnbp->rc, &cut_connection);
        if(cut_connection) {
            send_debug_message(dnbp->logger, "Manage socket returned cut connection: %d \n", dnbp->rc->sock);
            int s = dnbp->rc->sock;
            close(s);
            if(dnbp->rc->write_wrapper) {
                close(dnbp->rc->write_wrapper->sock);
                dnbp->rc->write_wrapper->sock = 0x0;
            }
            dnbp->rc->connection_notice(dnbp->rc->con_pass_back, 0, NULL);
        }
    }
}
void shutdown_dart(dnbcon* dbc)
{
    int s = dbc->dnbp->rc->sock;
    close(s);
    otl* o = dbc->dnbp->logger;
    send_debug_message(o, "Life cycle master shutdown starting.\n");
    stop_logging(o);
    stop_system(dbc->cntr->ass);
    join_n_clean(dbc->cntr->ass);
...
void parse_con_nb(int *cut_con, rex_con *rc)
{
    short ord_type = 0;
    dnbparse* pc = (dnbparse*)rc->parser;
    ...
    while (should_parse && bytes_used < len) {
        ...
        switch (ord_type) {
            ...
            case 0x00F1:
                add_seq = 1;
                nb_on_rej(pc, d_off, mess_len);
                break;
            case 0x0081:
                add_seq = 1;
                nb_on_fill(pc, d_off, mess_len);
                break;
            case 0x0101:
                add_seq = 1;
            ...
            case 0x00C1:
                add_seq = 1;
                parse_vfill_message(pc, d_off, mess_len);
                break;
            case 0x0141:
                bytes_used = mess_len;
                should_parse = 0;
...
void send_nb_logon(void* dd)
{
    ...
    pa->in_seq = 1;
    pa->nyse->sock = pa->rc->sock;//read_wrapper->sock;
    ...
    *logon->wr_ptr++ = '\0';
    ...
    databuf_memcpy(logon, pa->nyse->ex->username, pa->nyse->ex->ulen);
    int cp_len = pa->nyse->ex->ulen;
        ...
        dnbcntr* cntr)
{
    ...
    oats->ex->username= calloc(cntr->mc->mod_name_len + 4, 1);
    oats->ex->password= calloc(cntr->mc->mod_name_len + 4, 1);
    memcpy(oats->ex->username, "ROM", 3);
    memcpy(oats->ex->password, "ROM", 3);
    memcpy(oats->ex->username + 3, cntr->mc->mod_name, 
            cntr->mc->mod_name_len);
    oats->ex->ulen = cntr->mc->mod_name_len + 3;
    memcpy(oats->ex->password + 3, cntr->mc->mod_name, 
            cntr->mc->mod_name_len);
    oats->ex->plen = cntr->mc->mod_name_len + 3;
        ...
        char* config, int cfglen, char* user, int ulen)
{
    ...
    ci->mod_name = calloc(utp_len + 1, 1);
    memcpy(ci->mod_name, utpnum, utp_len);
    ci->mod_name_len = utp_len;
    ci->o = get_logger(cntr->ass);
    ci->ex->username = cntr->mc->username;
    ci->ex->ulen = cntr->mc->ulen;
    ci->gk = cntr->gk;
    ci->p = cntr->ass;
    ...
    dnbparse* uparse = create_nb_parser(ci->o, ci, li, dnbc);
    ci->logon = li;
    rex_con* rc = create_rex_con(ci->p, ci->ex, uparse, uparse,li,
            ...
            parse_con_nb, NULL);
    rc->parser = uparse;
    rc->connect = send_nb_logon;
    uparse->rc = rc;
    dnbc->dnbp = uparse;
    ci->rc = rc;
    rex_direct_connect(ci->p, rc);
        ...
        void(*on_connect)(uint8_t is_connected))
{
    ...
    dbc->cntr->mc->mod_name_len = mod_name_len;
    ...
    dbc->dnbp = create_nyseb_con(dbc, dbc->cntr,dbc->cntr->mc->mod_name,
            dbc->cntr->mc->mod_name_len, user, ulen);
    con_int* ci = create_empty_interface();
    ci->can_send_orders = 1;
    ci->connected = 1;
    ci->sock = 0;
