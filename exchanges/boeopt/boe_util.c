...
/* Boe prices are 10 digit strings of which the first 6 characters
   are the whole number and the last 4 are the decimal part. */
#define BOEPRICE_WHOLENUM_DIGITS 6
#define BOEPRICE_DECIMAL_PLACES 4
                  ...
                  char *boeprice)
{
    ...
    if (nwhole < 7) {
        boeprice += BOEPRICE_WHOLENUM_DIGITS - nwhole;
        if (nwhole > 0) {
            memcpy(boeprice, pricein, nwhole);
            boeprice += nwhole;
        }
        pricein = decimalpt;
        ...
        if (pricein != end) {
            ...
            if (ndecimal < 5) {
                memcpy(boeprice, pricein, ndecimal);
                retval = 0;
            }
        } else {
            retval = 0;
...
{
    int retval = -1;
    const char *decimalpart = pricein + BOEPRICE_WHOLENUM_DIGITS;
    ...
    if (pricein != decimalpart
        ...
        const char *end = decimalpart + BOEPRICE_DECIMAL_PLACES;
...
/* The chief assumption of the following numeric field functions is
   that the boe numeric field has already been initialized to all '0'
                    ...
                    size_t boe_field_length)
{
    ...
    /* We seem to have a problem in that BOE will simply drop our
    ...
    if (in_length <= boe_field_length) {
        if (in_length > 0) {
            boe_field = (boe_field + boe_field_length) - in_length;
            memcpy(boe_field, in, in_length);
        }
...
int boe_get_numeric(const char *boe_field, size_t boe_field_length,
                    struct databuf *out, int zero_as_empty_string)
{
    int retval = -1;
    const char *end = boe_field + boe_field_length;
    while (*boe_field == '0' && boe_field != end) {
        ++boe_field;
    }
    if (boe_field != end) {
        if (databuf_memcpy(out, boe_field, end - boe_field) != -1) {
            retval = 0;
        }
                                          ...
                                          size_t ntimestamp)
{
    ...
    /* Can't call strtol on the time stamp as it sits in the BOE
...
{
    struct boe_timestamp_converter *retval = malloc(sizeof *retval);
    tzset();
    if (timezone == 18000)
