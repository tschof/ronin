...
/* TODO: This entire implementation looks dicey.  linux_only_dart_send
 * returns -1 upon error and 0 if it would block.  We do not check the
 * return for -1, which should IMMEDIATELY indicate us cutting the
 * connection.  And if it blocks 40 times (why 40?) then we just
 * return.  linux_only_dart_send only sets the disconnect flag, it
 * does not actually kill the connection, and I think it should. */
static int boe_sendMessage(pthread_mutex_t * mutex, const char *p,
        size_t n, const char *name, int name_len)
{
    const char *end = p + n;
    int nfail = 0, nwritten;
        ...
        size_t sendiovn)
{
    ...
    while (sendiovp != endiovp && nfailed < 32) {
        ...
        struct parse_con *connection)
{
    ...
    int nclordid = create_multiday_unique_order_token(ei->token_creator,
            modifyOrder.
            fixedFields.clOrdID,
            sizeof modifyOrder.
            fixedFields.clOrdID -
...
int setMaturity(dart_order_obj * csvmsg, struct EnterOrder *enterOrder,
        err * e)
{
    int retval = -1, n;
    e->message = "maturity date";
    char *p = get_rom_field(csvmsg, ROM_EXPIRE_DATE, &n);
    char maturityDate[9];
    if (n == 6) {
        memcpy(maturityDate, p, 6);
        p = get_rom_field(csvmsg, ROM_MATURITY_DAY, &n);
        if(n > 0) {
            if (n == 1) {
                memcpy(maturityDate + 6, "0", 1);
                memcpy(maturityDate + 7, p, 1);
            } else {
                memcpy(maturityDate + 6, p, 2);
            }
            maturityDate[8] = '\0';
            retval = setOptionalBinary(&enterOrder->optionalFields,
...
int setStrike(dart_order_obj * csvmsg, struct EnterOrder *enterOrder,
        err * e)
{
    int retval = -1, n;
    e->message = "strike price";
    char *p = get_rom_field(csvmsg, ROM_STRIKE, &n);
    if (n > 0) {
        retval = setOptionalPrice(&enterOrder->optionalFields,
                EnterOrderStrikePrice, p, n);
    }
    return retval;
}

int setPutCallIndicator(dart_order_obj * csvmsg,
        struct EnterOrder *enterOrder, err * e)
{
    int retval = -1, n;
    e->message = "put/call indicator";
    char *p = get_rom_field(csvmsg, ROM_CALL_PUT, &n);
    if (n == 1) {
        char c = 0;
        if (p[0] == 'P') {
            c = '0';
        ...
        if (c != 0) {
            retval = setOptionalAlphaNumeric(&enterOrder->optionalFields,
                    EnterOrderPutOrCall, &c, 1,
                    1);
        }
    }
    return retval;
...
int setOpenCloseIndicator(dart_order_obj * csvmsg,
        struct EnterOrder *enterOrder, err * e)
{
    int n;
    e->message = "invalid open/close indicator";
    char *p = get_rom_field(csvmsg, ROM_OPEN_CLOSE, &n);
    if (n == 1) {
        if (p[0] == '0') {
            p = "C";
        } else if (p[0] == '1') {
    ...
    return setOptionalAlphaNumeric(&enterOrder->optionalFields,
            EnterOrderOpenClose, p, n, 1);
}

int setCMTA(dart_order_obj * csvmsg, struct EnterOrder *enterOrder,
        err * e)
{
    e->message = "CMTA Number";
    int retval = 0, n;
    char *p = get_rom_field(csvmsg, ROM_CMTA, &n);
    if (n > 0) {
        retval = setOptionalBinary(&enterOrder->optionalFields,
                EnterOrderCMTANumber, p, n, 4);
    }
    return retval;
}

int setEquityOptionFields(dart_order_obj * csvmsg,
        struct EnterOrder *enterOrder, err * e)
{
    int retval = -1;
    e->message = "invalid option fields";
    if (setMaturity(csvmsg, enterOrder, e) != -1
            && setStrike(csvmsg, enterOrder, e) != -1
            && setPutCallIndicator(csvmsg, enterOrder, e) != -1
            && setOpenCloseIndicator(csvmsg, enterOrder, e) != -1
            && setCMTA(csvmsg, enterOrder, e) != -1) {
        retval = 0;
    }
    return retval;
}

int setCapacity(dart_order_obj * csvmsg, struct EnterOrder *enterOrder,
        err * e)
{
    e->message = "invalid capacity";
    int n, retval = -1;
    char *p = get_rom_field(csvmsg, ROM_CAP, &n);
    if (n != 1) {
        goto done;
    }
    if (p[0] == 'B') {
        ...
        struct parse_con *connection)
{
    ...
    if (setEquityOptionFields(csvmsg, &enterOrder, err) == -1) {
        goto cleanup;
    }
    ...
    /* Routing instructions are not mapped, but also not rejected. */

    /* All BATS Options orders are set to reject if they are priced to
     * lock or cross the NBBO.  This is done by setting the
     * DisplayIndicator to "R". */
    if (setOptionalAlphaNumeric
            (&enterOrder.optionalFields, EnterOrderDisplayIndicator, "R", 1,
             1) == -1) {
        goto cleanup;
    }
    ...
    struct dstring acct;
    if (acctRule != 0 && acctRule->convert(acctRule, csvmsg, &acct) == 0
            && setOptionalAlphaNumeric(&enterOrder.optionalFields,
                EnterOrderClearingAcct,
                acct.p, acct.nlen, 4) == -1) {
        err->message = "Setting ClearingAccount";
        goto cleanup;
    }
    ...
    p = get_rom_field(csvmsg, ROM_CLR_ACC, &n);
    int cap_len = 0;
    char *cap = get_rom_field(csvmsg, ROM_CAP, &cap_len);
    if (cap[0] == 'S') {
        int mm_id_len = 0;
        char* mm_id = get_rom_field(csvmsg, ROM_MM_ACC, &mm_id_len);
        if(mm_id_len <= 0) {
        setOptionalAlphaNumeric(&enterOrder.optionalFields,
                EnterOrderClearingAcct,
                p, (n > 4 ? 4: n), 4);
    ...
    if (setCapacity(csvmsg, &enterOrder, err) == -1) {
        goto cleanup;
    }
    ...
    /* ExecInst is ignored unless it specifies an ISO Sweep order
    ...
    int nclordid = create_multiday_unique_order_token(ei->token_creator,
            ...
            1);
    cond_error_report(nclordid != -1, err, "Internal error generating ID");
    set_rom_field(csvmsg, ROM_CLORDID, enterOrder.fixedFields.clOrdID,
            nclordid);
...
static struct init_args *loadConfig(const char *filename, int file_len)
{
    ...
    struct conf_map mappings[] = {
        ...
        {"BOE", "ClearingAccount", &accountp, 0}
    };
...
static orderbook *boe_book_init(struct TradingInterface *ei)
{
    ...
    orderbook *book = no_order_mng_book(generate_unique_token,
            ...
            0, 0,
            ei->
            exchange_conf->order_allocation,
...
static int boe_configure(struct TradingInterface *ei, const char *filename)
{
    ...
    if (ei->exchange_conf != 0
        ...
        ei->token_creator = init_order_token_creator();
...
static const char *csv_heartbeat_message = "H\n";

static romcollection_retval send_rom_heartbeat(const char *nameptr,
        size_t namelen,
        void *callback_token)
{
    romcollection_retval retval = okay;
    struct ob *book = callback_token;
    boe_sendMessage(&book->send_mutex, csv_heartbeat_message, 2, nameptr,
            namelen);
    return retval;
}

static romcollection_t *roms = 0;

static int boe_sendHeartbeat(struct TradingInterface *ei)
{
    /* Send heartbeat to the BATS system. */
    boe_sendMessage(&ei->book->send_mutex, (char *) &clientHeartbeat,
            sizeof clientHeartbeat, ei->book->fix_con_name,
            ei->book->fix_con_len);
    /* Send heartbeats to every connected ROM */
    romcollection_foreach(roms, send_rom_heartbeat, ei->book);
    return 0;
}
...
enum trade_disposition { new_trade = 0, corrected_trade, cancelled_trade };
typedef enum trade_disposition trade_disposition_t;
...
struct StatusFields {
    ...
    rdonlystr_t desc;
    rdonlystr_t side;
    rdonlystr_t correctedPrice;
    cleanup_callback cb;
    void *privateData1;
    void *privateData2;
    trade_disposition_t trade_disposition;
};
...
static void sendStatusMessageViaEmail(const struct StatusFields *sf)
{
    ...
    const char *title = "Trade received on unknown order";
    if (sf->trade_disposition == corrected_trade) {
        title = "Trade correction";
        databuf_write(email, "%s\nOriginal order information:", title);
    } else if (sf->trade_disposition == cancelled_trade) {
        title = "Trade busted";
        databuf_write(email, "%s\nOriginal order busted:\n", title);
    } else {
        databuf_write(email, "%s\n", title);
    }
    if (sf->timestamp.n > 0) {
    ...
    if (sf->trade_disposition == corrected_trade) {
        databuf_write(email, "Corrected price: %.*s\n",
                (int) sf->correctedPrice.n, sf->correctedPrice.p);
    }
...
/* Called when we receive a message referring to an order we cannot
 * find on our book. */
static void summarize_trade(char side, uint32_t shares, uint64_t px,
        databuf_t * buf)
{
    if (side == '1') {
        databuf_write(buf, "Bought ");
    } else if (side == '2') {
        databuf_write(buf, "Sold ");
    } else {
        if (isprint(side) != 0) {
            databuf_write(buf, "Unknown side (%c) ", side);
        } else {
            databuf_write(buf, "Unknown side (0x%02x) ", (int) side);
        }
    }
    databuf_write(buf, "%u shares at $", shares);
    binaryPriceToString(px, buf);
}

static int unknownOrder(struct BoeHeader *header)
{
    const char *title = 0;
    databuf_t *email = databuf_alloc(512);
    if (header->messageType == 0x12) {
        /* Trade Cancel or Correct */
        struct OrderBustOrCorrection *adjustment =
            (struct OrderBustOrCorrection *) header;
        databuf_write(email,
                "The trade with execution ID %lu for the order "
                "with ClOrdID %.*s ", adjustment->execRefID,
                (int) strnlen(adjustment->responseHeader.clOrdID,
                    sizeof adjustment->responseHeader.
                    clOrdID),
                adjustment->responseHeader.clOrdID);
        if (adjustment->correctedPx == 0) {
            databuf_reset(email);
            title = "BATS Options Binary Trade Bust";
            databuf_write(email,
                    "The following trade has been BUSTED:\n\n");
            summarize_trade(adjustment->side, adjustment->lastShares,
                    adjustment->lastPx, email);
            databuf_write(email, "\n\n");
        } else {
            databuf_reset(email);
            title = "BATS Options Binary Trade Adjusted";
            databuf_write(email,
                    "The trade indicated below has had its price adjusted from $",
                    adjustment->execRefID,
                    (int) strnlen(adjustment->responseHeader.clOrdID,
                        sizeof adjustment->responseHeader.
                        clOrdID),
                    adjustment->responseHeader.clOrdID);
            binaryPriceToString(adjustment->lastPx, email);
            databuf_write(email, " to $");
            binaryPriceToString(adjustment->correctedPx, email);
            databuf_write(email, ".\n\nThe trade BEFORE correction was: ");
            summarize_trade(adjustment->side, adjustment->lastShares,
                    adjustment->lastPx, email);
            databuf_write(email, ".\nThe trade AFTER correction is:   ");
            summarize_trade(adjustment->side, adjustment->lastShares,
                    adjustment->correctedPx, email);
            databuf_write(email, ".\n\n");
        }
        databuf_write(email, "ClOrdID %.*s\n",
                (int) strnlen(adjustment->responseHeader.clOrdID,
                    ...
                    clOrdID),
                adjustment->responseHeader.clOrdID);
        databuf_write(email, "Execution ID %lu\n", adjustment->execRefID);
        databuf_write(email, "BATS Order ID %lu\n", adjustment->orderID);
        databuf_write(email, "\nThe original trade occurred at ");
        char timestamp[TIMESTAMP_LEN_WITH_MICROS + 1];
        if (binaryDateTimeToStr
                (adjustment->origTime, timestamp, sizeof timestamp) != -1) {
            databuf_write(email, "%.*s\n", (int) TIMESTAMP_LEN_WITH_MICROS,
                    timestamp);
        }
    } else if (header->messageType == 0x11) {
        /* Trade */
        title = "BATS Options Binary Trade";
        databuf_write(email,
                "Received a trade for an order that we do not have on our order book.\n\n");
        struct OrderExecution *trade = (struct OrderExecution *) header;
        summarize_trade(0, trade->lastShares, trade->lastPx, email);
        databuf_write(email, "\nClOrdID %.*s\n",
                (int) strnlen(trade->responseHeader.clOrdID,
                    sizeof trade->responseHeader.clOrdID),
                trade->responseHeader.clOrdID);
        databuf_write(email, "ExecID %lu\n", trade->execID);
        databuf_write(email, "LeavesQty %u\n", trade->leavesQty);
        databuf_write(email, "Base Liquidity Indicator %c\n",
                trade->baseLiquidityIndicator);
        if (isprint(trade->subLiquidityIndicator) != 0) {
            databuf_write(email, "Sub Liquidity Indicator %c\n",
                    trade->subLiquidityIndicator);
        }
        databuf_write(email, "Contra Broker '%.*s'\n",
                (int) strnlen(trade->contraBroker,
                    sizeof trade->contraBroker),
                trade->contraBroker);
        databuf_write(email, "Trade executed at ");
        char timestamp[TIMESTAMP_LEN_WITH_MICROS + 1];
        if (binaryDateTimeToStr
                (trade->responseHeader.transactionTime, timestamp,
                 sizeof timestamp) != -1) {
            databuf_write(email, "%.*s\n", (int) TIMESTAMP_LEN_WITH_MICROS,
                    timestamp);
        }
    }
    if (databuf_unread(email) > 0) {
        if (title == 0) {
            title = "Unknown event";
        }
        send_biz_message(email->rd_ptr, databuf_unread(email), title,
        ...
        case 0x0C:
            {
                ...
                set_rom_field(orderobj, ROM_CLORDID,
                        ...
                        strnlen(orderModified->responseHeader.clOrdID,
                            sizeof orderModified->
                            responseHeader.clOrdID));
        ...
        case 0x12:
            {
                ...
                if (bustCorrection->correctedPx == 0) {
                    sf.execShares.p = dbuf.wr_ptr;
        ...
        int *drop, struct parse_con *pc)
{
    ...
    if (header->messageType == 0x07) {
        size_t messageLength = getFramingLength(header);
        if (messageLength < sizeof(struct BoeLoginResponse)) {
            send_debug_message
...
#define chk_report_stderr(cond, msg)                                    \
    if (!(cond)) {                                                      \
        fprintf(stderr, "%s:%d ERROR: %s\n", __FILE__, __LINE__, (msg)); \
    }

/* We use this to track the coming and going of ROM connections. */
static void boeopt_rom_on_connection_update(con_obj * connection,
        int is_connected, char *log,
        int len)
{
    const char *connection_name = get_name(connection);
    const size_t connection_name_length = get_name_len(connection);
    /* We are specifically looking for an is_connected value of 1.  2
     * is also possible and signifies a socket connection; 1 is sent
     * after successfully exchanging login messages and is the
     * indicator we want to track when deciding whom to send a
     * heartbeat message to. */
    if (is_connected == 1) {
        printf("INFO:  %.*s has connected (is_connected %d)\n",
                (int) connection_name_length, connection_name,
                is_connected);
        chk_report_stderr(romcollection_addrom
                (roms, connection_name,
                 connection_name_length) == 0,
                "Adding ROM name to collection");
    } else if (is_connected == 0) {
        printf("INFO:  %.*s has disconnected (is_connected %d)\n",
                (int) connection_name_length, connection_name,
                is_connected);
        chk_report_stderr(romcollection_removerom
                (roms, connection_name,
                 connection_name_length) == 0,
                "Removing ROM name from collection");
    }
    free((char *) connection_name);
}
...
static struct BoeInterface *BoeInterface_create(void)
{
    ...
    boe->base.server_connection.on_connection_update =
        boeopt_rom_on_connection_update;
...
#define chk_exit_on_fail(cond, msg, progreturn)                         \
    if (!(cond)) {                                                       \
...
int main(int argc, char *argv[])
{
    int retval = 0;
    if (argc > 1) {
        chk_exit_on_fail(roms = make_romcollection(),
                "Failed to make ROM collection", 1);
