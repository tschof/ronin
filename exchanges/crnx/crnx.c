...
static int is_crnx_connected = 1;
...
static char *crnx_generate_id(size_t * nlen)
{
    char *cl_ord_id = 0;
    if (nlen != 0) {
        int retval = 0;
...
int get_is_crnx_connected()
{
    int ret = 0;
    pthread_mutex_lock(&connection_lock);
    ret = is_crnx_connected;
...
void set_is_crnx_connected(int is)
{
    pthread_mutex_lock(&connection_lock);
    is_crnx_connected = is;
    pthread_mutex_unlock(&connection_lock);
}

int set_crnx_defaults(char *rommsg, struct dart_fix_pos *map,
                        char *mtype, databuf_t * output,
                        size_t romfield, int *rejected,
                        dart_ex_order_obj * order)
{
    int result = fixmsg_addstr_bylen(output, "21", "1", 1);
    return result;
}

int crnx_fixrom_status_xlate(char *fixmsg, dart_rom_pos * positions,
...
long crnx_send_alert_for_message(char *fix_message,
                            long fix_len,
...
int rom_to_crnx_side(char *rom, romindex_t * map, char *type,
                      databuf_t * fix, size_t field, int *rejected,
                      dart_ex_order_obj * order);
int rom_to_crnx_type(char *rom, romindex_t * map, char *type,
                      databuf_t * fix, size_t field, int *rejected,
                      dart_ex_order_obj * order);
...
int send_crnx_logon(struct parse_args *pa);
int set_crnx_exTag(char *mess, struct dart_fix_pos *positions,
                     char *mtype, databuf_t * output, size_t i,
                     int *rejected, dart_ex_order_obj * ob);

size_t create_crnx_fix_header(databuf_t * fixmsg,
...
int crnx_symbol_hack(char *rommsg, dart_fix_pos * map,
...
int crnx_main()
{
    ...
    ex = init_config("crnx.dat", 16);
    fix_args = load_fix_config("crnx.dat", 16);
    ...
    ob = init_order_book(crnx_generate_id, get_can_send_orders,
                         def_rom2fix, set_crnx_defaults,
    ...
    ret =
        dart_connect(&from_fix_func, ex->mod_name, ex->mod_name_len,
                     ...
                     fix_args, send_crnx_logon, build_fix_offsets,
                     low_activity_sig, 1);
    ...
    while (keep_running) {
        ...
        if (newtime->tm_hour == ex->end_hour
            ...
            if (!get_is_crnx_connected()) {
                set_is_crnx_connected(1);
                fix_args->reset_time->create_new = 1;
                ...
                ret =
                    dart_connect(&from_fix_func, ex->mod_name,
                                 ...
                                 send_crnx_logon, build_fix_offsets,
                                 low_activity_sig, 1);
            }
        }
    }
...
int send_crnx_logon(struct parse_args *pa)
{
    ...
    if (mblk != NULL) {
        size_t len = sprintf(mblk,
                             "553=%s%c554=%s%c108=30%c98=0%c141=Y%c",
                             user,
                             FIX_FIELD_DELIM,
                             password,
                             FIX_FIELD_DELIM,
                                    ...
                                    char *log, int len)
{
    ...
    set_is_crnx_connected(is_connected);
                      ...
                      dart_ex_order_obj * order)
{
    ...
    if (len > 0) {
        char *p = rom + map[field].beg;
        char* max = rom;
        int max_len = map[34].end - map[34].beg;
        max += map[34].beg;
        if(max_len > 0){  
            set_val_in_order_struct("Z", 1, field, order, type);
            result = fixmsg_addstr_bylen(fix, map[field].name, "Z", 1);
        } else if(p[0] == '1') {
            set_val_in_order_struct("C", 1, field, order, type);
            result = fixmsg_addstr_bylen(fix, map[field].name, "C", 1);
        } else if(p[0] == '2') {
            set_val_in_order_struct("F", 1, field, order, type);
            result = fixmsg_addstr_bylen(fix, map[field].name, "F",1);
        } else {
            set_val_in_order_struct(p, len, field, order, type);
            result = fixmsg_addstr_bylen(fix, map[field].name, p, len);
        }
