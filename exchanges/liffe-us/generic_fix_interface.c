...
/* Each project should put customized values and function declarations
                               ...
                               struct fix_interface *handler)
{
    ...
    if (handler != 0) {
        ...
        if (result == 0) {
            database_init(&exchange_config->db, exchange_config->dest_id,
                          create_queue(), pbook->cl_ord_id_map,
                          pbook->rom_ord_id_map, pbook);
...
int main(int argc, char **argv)
{
    ...
    /* About the first thing to do is to call the
       generic_fix_interface_main() function to pass control to the
       destination-specific code to setup the FIX destination
       interface handler. */
...
void con_callBack(con_obj * co, int is_connected, char *log
                  __attribute__ ((unused)), int len
                  __attribute__ ((unused)))
{
