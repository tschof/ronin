...
int ad_is_connected = 0;

int get_is_ad_connected() 
{
    return ad_is_connected;
}

void place_holder_from_ad(struct message *mess, void *ob)
{
}
void place_holder_from_rom_func(struct message *mess, void *b)
{
}
unsigned long build_ad_offsets(char *data, unsigned long size,
...
void ad_connection_callBack(con_obj * co, int is_connected,
                                    char *log, int len)
{
    ad_is_connected = is_connected;
    char *name = (char *) get_name(co);
    send_debug_message("Info about this FIX Connection: %s, Is it connected {%d} ? \n",
           name, is_connected);
    if (name) {
        free(name);
    }
...
{
    char *name = (char *) get_name(co);
    send_debug_message("Info about this Connection: %s, Is it connected {%d} ? \n",
           name, is_connected);
    if (name) {
        free(name);
    }
...
int main(int argc, char **argv)
{
    ...
    orderbook *ob = NULL;
    rom_q = create_queue();
    ex = init_config("arcadirect.dat", 14);
    fix_args = load_fix_config("arcadirect.dat", 14);
    load_rom_config(ex->dc, rom_q);
    rom_parser_helper* rph = create_rph(ph_build_dart_obj);
    ob = init_ad("arcadirect.dat", fix_args,
                    ex,  (&get_is_ad_connected));
    ...
    ret =
        start_dart_client_services_2(&place_holder_from_rom_func, ex->mod_name,
                                   ...
                                   rph, copy_rom_parser_helper,ad_rom_helper_func);
    ret =
        dart_connect(&place_holder_from_ad, ex->mod_name, ex->mod_name_len,
                     ex->fix_port, ex->fix_ip, ex->fix_ip_len,
                     ex->end_hour, ex->end_min, 7, ex->heart_beat_int, ob,
                     ad_connection_callBack,parse_addirect,
                     fix_args, send_ad_logon, build_ad_offsets,
                     low_activity_sig, 1);
    ...
    while (keep_running) {
        ...
        if (newtime->tm_hour == ex->end_hour
            ...
            if (!ad_is_connected) {
                ad_is_connected = 1;
                ret =
                    dart_connect(&place_holder_from_ad, ex->mod_name,
                                 ex->mod_name_len, ex->fix_port,
                                 ex->fix_ip, ex->fix_ip_len, ex->end_hour,
                                 ex->end_min, 7, ex->heart_beat_int, ob,
                                 ad_connection_callBack,
                                 parse_addirect, fix_args,
                                 send_ad_logon, build_ad_offsets,
                                 low_activity_sig, 1);
            }
        }
    }
