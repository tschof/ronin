...
unsigned long dsr_parse_data(sequence_req * sr, char *data,
                             ...
                             unsigned long *seq_num)
{
    return sr->parse_messages(data, size, bytes_used, seq_num);
}
...
int ll_before_reset_time(struct init_args* in, time_t now)
{
    ...
    if(in->reset_time->reset_day == 7 && (day_offset > 1 && day_offset < 7)) {
        create_new = 1;
    } else if(in->reset_time->reset_day == 7 || in->reset_time->reset_day == ti->tm_wday) {
        int sec_diff = ((lm.tm_hour * 3600) + (lm.tm_min * 60) + lm.tm_sec)  -
                                                           ...
                                                           size_t * len))
{
    ...
    if (sr != NULL) {
        ...
        if(in->reset_time->create_new == 0 && in->reset_time->last_logon != 0
                && in->reset_time->reset_day >= 0 && in->reset_time->reset_day < 8) {
            sr->create_new = ll_before_reset_time(in, rawtime);
            send_debug_message("Name: %s, Create new? %d last_logon? %d Reset day: %d, hour: %d, min: %d \n",
                               sr->name, sr->create_new, in->reset_time->last_logon,
                               in->reset_time->reset_day, in->reset_time->reset_hour,
                               in->reset_time->reset_min);
        }

        in->reset_time->last_logon = rawtime;
        sr->incoming = 0;
        sr->outgoing = 0;
