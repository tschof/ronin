...
void drr_create_resend_out(resend_req * rr, FILE * fp)
{
    size_t bytes_read = 0;
    size_t bytes_used = 0;
    ...
    while (should_parse && !feof(fp)) {
        ...
        bytes_read = fread(wr_ptr, 1, RESEND_REQ_FILE_READ_CHUNK, fp);
    ...
    fseek(fp, beg_off, SEEK_SET);
    read_bytes = fread(rr->output, 1, cp_len, fp);
    while (read_bytes < cp_len && !feof(fp)) {
        char *off = rr->output;
        size_t b_left = cp_len - read_bytes;
        size_t r_bytes = 0;
        off += read_bytes;
        r_bytes = fread(off, 1, b_left, fp);
        read_bytes += r_bytes;
    }
    rr->len = read_bytes;
