    ...
    if (handler != 0) {
        ...
        if (result == 0) {
            database_init(exchange_config->db_user_name,
                          ...
                          pbook);
            result = handler->on_start(exchange_config, pbook);
        }
    }
    return result;
}
...
void default_on_session_status_changed(con_obj * connection,
                                       ...
                                       int len)
{
    (void) printf("FIX session %s %sestablished\n", get_name(connection),
                  is_established ? "" : "NOT ");
    if (is_established == 0) {
        fix_connection_callback(connection, is_established);
    }
    set_is_genericfix_connected(is_established);
}

static const struct fix_destination_interface
    fix_destination_interface_defaults = {
    .config_file_name = "config.dat",
    .file_name_length = 10,
    .on_start = 0,
    .on_connect = send_logon,
    .on_session_status_change = default_on_session_status_changed,
    .pre_translate_func = 0,
    .id_generator = 0,
};
...
int main(int argc, char **argv)
{
    ...
    rom_q = create_queue();
    ex = init_config(handler.config_file_name, handler.file_name_length);
    fix_args =
        load_fix_config(handler.config_file_name,
                        handler.file_name_length);
    if (fix_args != 0) {
        printf("create_new flag is %d\n",
               fix_args->reset_time->create_new);
    }
    ...
    ret =
        dart_connect(&from_fix_func, ex->mod_name, ex->mod_name_len,
                     ...
                     handler.on_session_status_change, parse_fix_message,
                     fix_args, handler.on_connect, build_fix_offsets,
                     low_activity_sig, 1);
            ...
            if (!get_is_genericfix_connected()) {
                ...
                ret =
                    dart_connect(&from_fix_func, ex->mod_name,
                                 ...
                                 ex->heart_beat_int, ob,
                                 handler.on_session_status_change,
                                 parse_fix_message, fix_args,
                                 handler.on_connect, build_fix_offsets,
                                 low_activity_sig, 1);
            }
        }
    }
