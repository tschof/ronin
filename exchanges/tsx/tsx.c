...
#endif
static char *tsx_user_id = 0;
static int tsx_id_len = 0;
static char *tsx_no_trade_key = 0;
static int tsx_ntk_len = 0;
static char *broker_num = 0;
static int broker_num_len = 0;

static int tsx_translate_symbol(char *rom, romindex_t * map, char *fixtype,
    ...
    if (symlen > 0) {
        ...
        if (symlen < sizeof buf) {
            ...
            /* Handle the case in which there is a suffix in
               ...
               Now that we go direct to the TSX we do not put
...
static int tsx_translate_ordtype(char *rom, romindex_t * map,
    ...
    if (n > 0) {
        if (n == 2 && p[0] == '1' && p[1] == '1') {
            ...
            if (execinstlen == 1) {
                ...
                case 'M':
                case 'R':
                    /* From my reading of the TSX FIX SPEC this is the
                     * only way to send peg orders. */
                    fixmsg_addstr_bylen(fix, "7723", "M", 1);
                    fixmsg_addstr_bylen(fix, "7726", "Y", 1);
                    p = "1";
                    n = 1;
                    break;
                }
...
static int tsx_translate_capacity(char *rom, romindex_t * map,  /*@unused@ */
...
static int tsx_trans_clearing(char *rom, romindex_t * map,
...
static int tsx_map_routing_instructions(char *rom, romindex_t * map,
                                        char *type, databuf_t * fix,
                                        size_t field, int *rejected,
                                        dart_ex_order_obj * order)
{
...
static int tsx_map_short_locate(char *rom, romindex_t * map,
                                char *type, databuf_t * fix, size_t field,
                                int *rejected, dart_ex_order_obj * order)
{
    int result = -1;
...
long tsx_set_status_for_cnrej(char *fix_message,
...
static int tsx_fixrom_status_xlate(char *fixmsg, dart_rom_pos * positions,
                                   ...
                                   hashmap_t ord_map)
{
    ...
    if (romfield == 14 && mtype != 'U') {
        ...
        len =
            tsx_set_status_for_cnrej(fixmsg, positions, status, rom,
                                     order);
    }
    return 1;
}
 ...
 */
static int tsx_translate_side(char *rom, romindex_t * map, char *type,
...
static int tsx_custom_init(ex_config * exchange_config, orderbook * pbook)
{
    int result = -1;
    idgenerator = init_order_token_creator();
    if (idgenerator != 0) {
        pbook->rom2fix[4] = tsx_translate_side;
        pbook->rom2fix[5] = tsx_translate_symbol;
        pbook->rom2fix[8] = tsx_translate_ordtype;
        pbook->rom2fix[10] = tsx_translate_capacity;
        /* TIF mapping? If we're doing Limit on Close there should be
           a TIF mapping in addition to an order type mapping. */
        pbook->rom2fix[12] = tsx_trans_clearing;
#ifdef ALLOW_DESTMAP
        pbook->rom2fix[13] = tsx_map_routing_instructions;
#endif
        pbook->rom2fix[17] = def_field_noop;
        pbook->rom2fix[58] = tsx_map_short_locate;

        pbook->fix2rom[14] = tsx_fixrom_status_xlate;
        ...
        /* Initialize the Destination->ExDest mapping mechanism. */
        {
            int nlen = -1;
            char *mapconfig =
                get_val_for_tag(exchange_config->dc, "TSX", 3, "DestMap",
                                7, &nlen);
    ...
    tsx_user_id =
        get_val_for_tag(exchange_config->dc, "FIX SERVER", 10,
                        "TSX USER ID", 11, &tsx_id_len);
    tsx_no_trade_key =
        get_val_for_tag(exchange_config->dc, "FIX SERVER", 10,
                        "TSX NT KEY", 10, &tsx_ntk_len);
    broker_num =
        get_val_for_tag(exchange_config->dc, "FIX SERVER", 10,
...
static int tsx_pre_translate(char *rom, romindex_t * map,
                             char *type, databuf_t * fix,
                             size_t field, int *rejected,
                             dart_ex_order_obj * order)
{
    databuf_write(fix, "%c6751=%s", FIX_FIELD_DELIM, tsx_user_id);
    databuf_write(fix, "%c7713=NM", FIX_FIELD_DELIM);
    databuf_write(fix, "%c7714=%s", FIX_FIELD_DELIM, tsx_no_trade_key);
    databuf_write(fix, "%c6774=%s", FIX_FIELD_DELIM, broker_num);
    return 0;
}

static char *tsx_fix_generate_id(size_t * nlen)
{
    char *cl_ord_id = 0;
    if (nlen != 0) {
        int retval = 0;
                               ...
                               struct fix_interface *handler)
{
    int result = -1;
    if (handler != 0) {
        handler->config_file = "tsx.dat";
        handler->onstart = tsx_custom_init;
        handler->pre_translate_func = tsx_pre_translate;
        handler->id_generator = tsx_fix_generate_id;
        result = 0;
    }
    return result;
}
