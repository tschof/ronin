...
/* In the latest iteration of the framework, the create_new flag is set by
 * the client of the framework; the framework no longer determines the value
 * of create_new on its own.
 *
 * For our purposes, we will determine the value of create_new based on the
 * last run date.  If the last run date is before the current date, we
 * consider everything to be a new session.  This is simply because this
 * particular connection will not run past midnight local time and each
 * startup will require new day processing to be invoked.
 */
int determine_create_new(init_args * args)
{
    int create_new = 0;
    ...
    if (now != -1) {
        ...
        if (ltm != 0) {
            char current_run_date[256];
            int nwritten = -1;
            memset(current_run_date, 0, sizeof current_run_date);
            sprintf(current_run_date, "%04d%02d%02d", ltm->tm_year + 1900,
                    ltm->tm_mon + 1, ltm->tm_mday);
            if ((nwritten < sizeof current_run_date) && (nwritten > -1)) {
                int nlen = -1;
                char *last_run_date =
                    get_val_for_tag(args->dc, "TIMES", 5, "Last Run Day",
                                    12, &nlen);
                if (last_run_date != 0 && nlen > 0) {
                    int cmpresult =
                        strcmp(last_run_date, current_run_date);
                    if (cmpresult != 0) {
                        create_new = 1;
                        if (cmpresult > 0) {
                            fprintf(stderr,
                                    "WARNING: Last Run Day in the future\n");
                            fprintf(stderr, "\tLast Run Day: %s\n",
                                    last_run_date);
                            fprintf(stderr, "\tCurr Run Day: %s\n",
                                    current_run_date);
                        }
                    }
                    free(last_run_date);
...
int main(int argc, char **argv)
{
    ...
    rom_q = create_queue();
    if ((ex = init_config(handler.config_file, handler.file_len)) == 0) {
        fprintf(stderr, "Cannot load config file %.*s\n", (int) handler.file_len,
                handler.config_file);
        exit(EXIT_FAILURE);
    }
